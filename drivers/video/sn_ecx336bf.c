/*
 * Borderless LCD Framebuffer driver Sony ECX336BF
 *
 * Copyright 2018-2020 Borderless Inc.
 * Licensed under the GPL-2 or later.
 */

#define DRIVER_NAME "sn-ecx336bf"
...
struct ecx336bf_info {
	struct fb_info *fb;
	struct device *dev;
	struct spi_driver spidrv;
	struct ecx336bf_disp_info *disp_info;
...
static int ecx336bf_control(struct spi_device *spi, unsigned char reg, unsigned short value)
{
	int ret;
...
static int ecx336bf_spidev_probe(struct spi_device *spi)
{
	int ret;
	struct spi_control *ctl;
	struct ecx336bf_info *info = container_of(spi->dev.driver,
						struct ecx336bf_info,
						spidrv.driver);
	...
	ret = ecx336bf_control(spi, LQ035_SHUT_CTL, LQ035_ON);
	ret |= ecx336bf_control(spi, LQ035_DRIVER_OUTPUT_CTL, ctl->mode);
	if (ret) {
		kfree(ctl);
		return ret;
	}
...
static int ecx336bf_spidev_remove(struct spi_device *spi)
{
	return ecx336bf_control(spi, LQ035_SHUT_CTL, LQ035_SHUT);
}

#ifdef CONFIG_PM
static int ecx336bf_spidev_suspend(struct spi_device *spi, pm_message_t state)
{
	return ecx336bf_control(spi, LQ035_SHUT_CTL, LQ035_SHUT);
}

static int ecx336bf_spidev_resume(struct spi_device *spi)
{
	int ret;
	...
	return ecx336bf_control(spi, LQ035_SHUT_CTL, LQ035_ON);
}
#else
# define ecx336bf_spidev_suspend NULL
# define ecx336bf_spidev_resume  NULL
#endif

/* Power down all displays on reboot, poweroff or halt */
static void ecx336bf_spidev_shutdown(struct spi_device *spi)
{
	ecx336bf_control(spi, LQ035_SHUT_CTL, LQ035_SHUT);
}

static int ecx336bf_backlight(struct ecx336bf_info *info, unsigned arg)
{
...
static int bfin_ecx336bf_calc_timing(struct ecx336bf_info *fbi)
{
...
static void bfin_ecx336bf_config_ppi(struct ecx336bf_info *fbi)
{
	unsigned ppi_pmode;
...
static inline void bfin_ecx336bf_disable_ppi(void)
{
	bfin_write_PPI_CONTROL(bfin_read_PPI_CONTROL() & ~PORT_EN);
}

static inline void bfin_ecx336bf_enable_ppi(void)
{
	bfin_write_PPI_CONTROL(bfin_read_PPI_CONTROL() | PORT_EN);
}

static void bfin_ecx336bf_start_timers(void)
{
	enable_gptimers(TIMER_VSYNCbit | TIMER_HSYNCbit);
}

static void bfin_ecx336bf_stop_timers(void)
{
...
static void bfin_ecx336bf_init_timers(struct ecx336bf_info *fbi)
{

	bfin_ecx336bf_stop_timers();
...
static void bfin_ecx336bf_config_dma(struct ecx336bf_info *fbi)
{
...
static inline void bfin_ecx336bf_free_ports(unsigned ppi16)
{
	if (ppi16)
...
static int bfin_ecx336bf_request_ports(struct platform_device *pdev,
				      unsigned ppi16)
{
	int ret;
...
static int bfin_ecx336bf_fb_open(struct fb_info *info, int user)
{
	struct ecx336bf_info *fbi = info->par;

	spin_lock(&fbi->lock);
	fbi->ecx336_open_cnt++;

	if (fbi->ecx336_open_cnt <= 1) {

		bfin_ecx336bf_disable_ppi();
		SSYNC();

		bfin_ecx336bf_config_dma(fbi);
		bfin_ecx336bf_config_ppi(fbi);
		bfin_ecx336bf_init_timers(fbi);

		/* start dma */
		enable_dma(CH_PPI);
		bfin_ecx336bf_enable_ppi();
		bfin_ecx336bf_start_timers();
		ecx336bf_backlight(fbi, 1);
	}
...
static int bfin_ecx336bf_fb_release(struct fb_info *info, int user)
{
	struct ecx336bf_info *fbi = info->par;
	...
	if (fbi->ecx336_open_cnt <= 0) {
		ecx336bf_backlight(fbi, 0);
		bfin_ecx336bf_disable_ppi();
		SSYNC();
		disable_dma(CH_PPI);
		bfin_ecx336bf_stop_timers();
...
static int bfin_ecx336bf_fb_check_var(struct fb_var_screeninfo *var,
				     struct fb_info *info)
{
	struct ecx336bf_info *fbi = info->par;
...
int bfin_ecx336bf_fb_cursor(struct fb_info *info, struct fb_cursor *cursor)
{
	if (nocursor)
		return 0;
	else
...
static int bfin_ecx336bf_fb_setcolreg(u_int regno, u_int red, u_int green,
...
static struct fb_ops bfin_ecx336bf_fb_ops = {
	.owner = THIS_MODULE,
	.fb_open = bfin_ecx336bf_fb_open,
	.fb_release = bfin_ecx336bf_fb_release,
	.fb_check_var = bfin_ecx336bf_fb_check_var,
	.fb_fillrect = cfb_fillrect,
	.fb_copyarea = cfb_copyarea,
	.fb_imageblit = cfb_imageblit,
	.fb_cursor = bfin_ecx336bf_fb_cursor,
	.fb_setcolreg = bfin_ecx336bf_fb_setcolreg,
};

static irqreturn_t bfin_ecx336bf_irq_error(int irq, void *dev_id)
{
	/*struct ecx336bf_info *info = (struct ecx336bf_info *)dev_id;*/
...
static int bfin_ecx336bf_probe(struct platform_device *pdev)
{
	struct ecx336bf_info *info;
	...
	ret = bfin_ecx336bf_calc_timing(info);
	if (ret < 0) {
	...
	fbinfo->fbops = &bfin_ecx336bf_fb_ops;
	...
	ret = bfin_ecx336bf_request_ports(pdev,
	...
	ret = request_irq(info->irq, bfin_ecx336bf_irq_error, 0,
	...
	info->spidrv.probe    = ecx336bf_spidev_probe;
	info->spidrv.remove   = ecx336bf_spidev_remove;
	info->spidrv.shutdown = ecx336bf_spidev_shutdown;
	info->spidrv.suspend  = ecx336bf_spidev_suspend;
	info->spidrv.resume   = ecx336bf_spidev_resume;
 ...
 out7:
	bfin_ecx336bf_free_ports(info->disp_info->ppi_mode ==
...
static int bfin_ecx336bf_remove(struct platform_device *pdev)
{
	struct fb_info *fbinfo = platform_get_drvdata(pdev);
	struct ecx336bf_info *info = fbinfo->par;
...
#ifdef CONFIG_PM
static int bfin_ecx336bf_suspend(struct device *dev)
{
	struct fb_info *fbinfo = dev_get_drvdata(dev);
	struct ecx336bf_info *info = fbinfo->par;

	if (info->ecx336_open_cnt) {
		ecx336bf_backlight(info, 0);
		bfin_ecx336bf_disable_ppi();
		SSYNC();
		disable_dma(CH_PPI);
		bfin_ecx336bf_stop_timers();
...
static int bfin_ecx336bf_resume(struct device *dev)
{
	struct fb_info *fbinfo = dev_get_drvdata(dev);
	struct ecx336bf_info *info = fbinfo->par;
	...
	if (info->ecx336_open_cnt) {
		...
		bfin_ecx336bf_config_dma(info);
		bfin_ecx336bf_config_ppi(info);
		bfin_ecx336bf_init_timers(info);

		/* start dma */
		enable_dma(CH_PPI);
		bfin_ecx336bf_enable_ppi();
		bfin_ecx336bf_start_timers();
		ecx336bf_backlight(info, 1);
	}

	return 0;
}

static struct dev_pm_ops bfin_ecx336bf_dev_pm_ops = {
	.suspend = bfin_ecx336bf_suspend,
	.resume  = bfin_ecx336bf_resume,
};
#endif

static struct platform_driver bfin_ecx336bf_driver = {
	.probe   = bfin_ecx336bf_probe,
	.remove  = bfin_ecx336bf_remove,
	.driver = {
		.name = DRIVER_NAME,
#ifdef CONFIG_PM
		.pm   = &bfin_ecx336bf_dev_pm_ops,
#endif
	},
};

module_platform_driver(bfin_ecx336bf_driver);
