...
extern int ql_i2c_read(u32 addr, u32 *val, u32 data_size);
extern int ql_i2c_write(long addr, long val, int data_size);
extern void ql_chip_init(struct msm_fb_data_type *mfd);
...
printk("eztest mdss_panel_pwr_ctl ---------->power state:%d ret:%d\n",power_state,ret);
	if (!ret)
		pinfo->panel_power_state = power_state;
				...
				size_t count, loff_t *ppos)
{
	...
	if ((*link_state) == DSI_HS_MODE){
		printk("eztest dsi state R***********DSI_HS_MODE\n");
		blen = snprintf(buffer, sizeof(buffer), "dsi_hs_mode\n");
	}else{
		printk("eztest dsi state R***********DSI_LP_MODE\n");
			...
			const char __user *p, size_t count, loff_t *ppos)
{
	...
	if (strnstr(input, "dsi_hs_mode", strlen("dsi_hs_mode"))){
		printk("eztest dsi state W***********DSI_HS_MODE\n");
		*link_state = DSI_HS_MODE;
	}else{
		printk("eztest dsi state W***********DSI_LP_MODE\n");
...
//			pr_debug("reset enable: pinctrl not enabled\n");
		...
		udelay(100);
		gpio_set_value(933, 1);
		udelay(15);
		ret = ql_i2c_read(0x204, &val, 4);
		printk("eztest ###################LP11 1.VX 204 read:0x%x err:%d\n", val, ret);
		ret = ql_i2c_read(0x700, &val, 4);
		printk("eztest ###################LP11 1.VX 700 read:0x%x err:%d\n", val, ret);
  //init client.
  ...
  //copy
		...
		udelay(200);
		ret = ql_i2c_read(0x204, &val, 4);
		printk("eztest ###################LP11 2.VX 204 read:0x%x err:%d\n", val, ret);
		ret = ql_i2c_read(0x700, &val, 4);
		printk("eztest ###################LP11 2.VX 700 read:0x%x err:%d\n", val, ret);
#endif
...
printk("eztest*******force hs bef read 0xac[%x]\n",tmp);
		tmp |= (1<<28);
		MIPI_OUTP((ctrl_pdata->ctrl_base) + 0xac, tmp);
printk("eztest*******force hs aft read 0xac[%x]\n",tmp);
		wmb();
	}
...
static int mdss_dsi_unblank(struct mdss_panel_data *pdata)
{
	...
	printk("eztest 1 --->%s+: ctrl=%p ndx=%d cur_blank_state=%d\n", __func__,
		ctrl_pdata, ctrl_pdata->ndx, pdata->panel_info.blank_state);
	...
	printk("eztest 2 --->%s+: ctrl=%p cur_blank_state=%d\n", __func__,
		ctrl_pdata, pdata->panel_info.blank_state);
	...
	printk("eztest 3 --->%s+: ctrl=%p dsp=%d cur_blank_state=%d\n", __func__,
		ctrl_pdata, pdata->panel_info.dynamic_switch_pending, pdata->panel_info.blank_state);
	if ((pdata->panel_info.type == MIPI_CMD_PANEL) &&
...
printk("eztest update mode **********h period:%d v period:%d************\n",hsync_period,vsync_period);
	current_dsi_v_total =
...
printk("eztest dfps **********h period:%d pDelay1:%d pDelay2:%d pllDelay:%d************\n",hsync_period,pipe_delay,pipe_delay2,pll_delay);
...
printk("eztest mdss dsi 1------------> byte_rate:%i\n", ctrl_pdata->byte_clk_rate);
printk("eztest mdss dsi 1------------> pclk_rate:%i\n", ctrl_pdata->pclk_rate);
...
printk("eztest mdss dsi 2------------> byte_rate:%i\n", ctrl_pdata->byte_clk_rate);
printk("eztest mdss dsi 2------------> pclk_rate:%i\n", ctrl_pdata->pclk_rate);
...
static int mdss_dsi_clk_refresh(struct mdss_panel_data *pdata)
{
	...
	printk("eztest %s frame:%d byte_clk_rate=%d pclk_rate=%d\n",
		__func__, pdata->panel_info.mipi.frame_rate, ctrl_pdata->byte_clk_rate, ctrl_pdata->pclk_rate);
	return rc;
}
			...
			struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	pr_info("%s: ctrl_base=%p ctrl_size=%x phy_base=%p phy_size=%x\n",
		...
		ctrl->phy_io.len);
	printk("eztest dsi*********** ctrl_base=%p ctrl_size=%x phy_base=%p phy_size=%x\n",
		ctrl->ctrl_base, ctrl->reg_size, ctrl->phy_io.base,
		ctrl->phy_io.len);
...
//	pinfo->mipi.dsi_phy_db.strength[0] = 6;// org 0xff
//	pinfo->mipi.dsi_phy_db.strength[1] = 0xff;// org 6
	printk("eztest dsi-reg*********str0:%d str1:%d*******\n",pinfo->mipi.dsi_phy_db.strength[0],pinfo->mipi.dsi_phy_db.strength[1]);
...
//	ctrl_pdata->disp_en_gpio = EN_V10_HUD;
printk("dsi_panel_device_register:eztest----->en gpio:%d\n",ctrl_pdata->disp_en_gpio);
...
//	ctrl_pdata->rst_gpio = SYS_RST_N;// enzo gpio override
printk("eztest mds dsi parse------------>gpio rst:%d\n",ctrl_pdata->rst_gpio);

	if (!gpio_is_valid(ctrl_pdata->rst_gpio))
		pr_err("%s:%d, reset gpio not specified\n",
						__func__, __LINE__);
//	else printk("dsi_panel_device_register:eztest----->rst gpio:%d\n",ctrl_pdata->rst_gpio);
	...
	printk("eztest mdss dsi -----------> pclk:%d bclk:%d splash:%d\n",ctrl_pdata->pclk_rate,ctrl_pdata->byte_clk_rate,pinfo->cont_splash_enabled);
