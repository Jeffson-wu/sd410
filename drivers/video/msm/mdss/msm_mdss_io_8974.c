			    ...
			    int frame_rate)
{
	...
	if (panel_info->mipi.data_lane0)
		lanes += 1;
printk("eztest 8974 *************clk div lane:%d format:%d***********\n",lanes,panel_info->mipi.dst_format);
...
printk("eztest 8974 ******pll clk:%d rate:%d**********\n",pll_divider_config.clk_rate,rate);
	...
	printk("eztest 8974 ***********mipi pclk:%d******\n",panel_info->mipi.dsi_pclk_rate);
	return 0;
}
