...
#if MONITOR_ENABLE
static struct delayed_work panel_monitor_work;
#endif
int panel_on = 0;
...
#if MONITOR_ENABLE
static void panel_monitor(struct work_struct *work)
{
	u32 val = 0;
	int rc;
	if(panel_on){
		  rc = ql_i2c_read(0x204, &val, 4);
		printk("eztest panel monitor?--------->1 204 read:0x%x err:%d\n", val, rc);
		ql_i2c_write(0x204, 0xFFFFFFFF, 4); 
		  rc = ql_i2c_read(0x204, &val, 4);
		printk("eztest panel monitor?--------->2 204 read:0x%x err:%d\n", val, rc);
//		  rc = ql_i2c_read(0x148, &val, 4);
//		printk("eztest panel monitor--------->aft 148 read:0x%x err:%d\n", val, rc);
//		  rc = ql_i2c_read(0x700, &val, 4);
//		printk("eztest panel monitor--------->aft 700 read:0x%x err:%d\n", val, rc);
  //init client.
//  QL_DBG("Turn around timeout counter register 0x%x\n", val);
//		ql_i2c_read(0x218, &val, 4); 
//		ql_i2c_read(0x204, &val, 4); 
//		ql_i2c_write(0x204, 0xFFFFFFFF, 4); 
		schedule_delayed_work(&panel_monitor_work,msecs_to_jiffies(MONITOR_INTERVAL));
	}
}
#endif

int mdss_dsi_panel_gpio_init(int en)
{
	int ret;

	ret = gpio_direction_output(EN_V1P8_HUD, en);
	printk("eztest mdss gpio----------->V1P8:%d set:%d\n",EN_V1P8_HUD,gpio_get_value(EN_V1P8_HUD));
	mdelay(1);
	ret = gpio_direction_output(PANEL_XCLR, en) + (ret << 1);
	mdelay(1);
	printk("eztest mdss gpio----------->1 XCLR:%d set:%d ret:%d\n",PANEL_XCLR,gpio_get_value(PANEL_XCLR),ret);
			...
			struct dsi_panel_cmds *pcmds)
{
	...
	printk("eztest panel**************link state:%d\n",pcmds->link_state);
...
printk("eztest dsi panel reset------->1 en:%d\n",enable);
...
#if 0
printk("eztest dsi panel reset------->2 en:%d\n",enable);

	if (!gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
		pr_debug("%s:%d, reset line not configured\n",
			   __func__, __LINE__);
	}
printk("eztest dsi panel reset------->3 en_gpio:%d rst_gpio%d\n",ctrl_pdata->disp_en_gpio,ctrl_pdata->rst_gpio);
...
printk("eztest dsi panel reset------->4 engpio:%d rstgpio:%d\n",ctrl_pdata->disp_en_gpio,ctrl_pdata->rst_gpio);
	if (enable) {
#if 0
...
#else
	mdss_dsi_panel_gpio_init(0);
	mdelay(10);
	mdss_dsi_panel_gpio_init(1);
	panel_hud_disp_on();

#endif
		printk("eztest panel reset ------------>bef ctrl state:%dF\n",ctrl_pdata->ctrl_state);
		if (ctrl_pdata->ctrl_state & CTRL_STATE_PANEL_INIT) {
			pr_debug("%s: Panel Not properly turned OFF\n",
						__func__);
			printk("eztest panel reset ------------>Not properly turned OFF\n");
			ctrl_pdata->ctrl_state &= ~CTRL_STATE_PANEL_INIT;
			pr_debug("%s: Reset panel done\n", __func__);
		}
		printk("eztest panel reset ------------>aft ctrl state:%dF\n",ctrl_pdata->ctrl_state);
		panel_on = 1;
#if MONITOR_ENABLE
		schedule_delayed_work(&panel_monitor_work,msecs_to_jiffies(MONITOR_INTERVAL));
#endif
...
printk("eztest end.dsi panel on----------->blank state:%d\n",pinfo->blank_state);
	pr_debug("%s:-\n", __func__);
	return 0;
}
...
printk("eztest end.dsi panel off----------->blank_state:%d\n",pinfo->blank_state);
	pr_debug("%s:-\n", __func__);
	return 0;
}
	...
	int enable)
{
	struct mdss_dsi_ctrl_pdata *ctrl = NULL;
	struct mdss_panel_info *pinfo;
printk("eztest 1.low pwr config----------->enable:%d\n",enable);
	if (pdata == NULL) {
		pr_err("%s: Invalid input data\n", __func__);
		return -EINVAL;
	}
...
printk("eztest 2.low pwr config----------->blank_state:%d\n",pinfo->blank_state);
	pr_debug("%s:-\n", __func__);
	return 0;
}
...
//	pcmds->link_state = DSI_HS_MODE;
	...
	printk("eztest panel pars**************link state:%s\n",data);
...
//	pinfo->lcdc.border_clr = 0xff;
	...
	printk("eztest panel ********traffic mode:%d \n",pinfo->mipi.traffic_mode);
	rc = of_property_read_u32(np,
...
//	mdss_dsi_panel_gpio_init(1,1);
//	mdelay(1);

#if 1
//	panel_hud_disp_on();
#else
printk("eztest-------->before spi register 20181003\n");
	rc = spi_register_driver(&spidrv);
	if (rc < 0) {
		pr_err("%s:%d register spi fail\n", __func__, __LINE__);
		return rc;
	}
printk("eztest-------->after spi register rc[%d] 20181003\n",rc);
#endif
#if MONITOR_ENABLE
	INIT_DELAYED_WORK(&panel_monitor_work, panel_monitor);
	msleep(1000);
#endif
