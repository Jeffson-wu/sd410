...
int qpnp_pon_set_restart_reason(enum pon_restart_reason reason)
{
	...
	printk("eztest qpnp pon set restart reason:%d",reason);
	if (!pon)
		return 0;
...
int qpnp_pon_system_pwr_off(enum pon_power_off_type type)
{
	...
	printk("eztest qpnp pon pwr off type:%d",type);
	if (!pon)
		return -ENODEV;
...
{
	struct qpnp_pon *pon = sys_reset_dev;
	printk("eztest qpnp pon is warn reset reason1:%d reason2:%d\n",pon->warm_reset_reason1,pon->warm_reset_reason2);
	if (!pon)
		return -EPROBE_DEFER;
...
static int qpnp_pon_store_and_clear_warm_reset(struct qpnp_pon *pon)
{
	...
	printk("eztest qpnp pon strore clear reset 1\n");
	rc = spmi_ext_register_readl(pon->spmi->ctrl, pon->spmi->sid,
	...
	printk("eztest qpnp pon strore clear reset 2\n");
	...
	printk("eztest qpnp pon strore clear reset 3\n");
	...
	printk("eztest qpnp pon strore clear reset 4\n");
...
qpnp_get_cfg(struct qpnp_pon *pon, u32 pon_type)
{
	...
	printk("eztest qpnp pon get config type:%d\n",pon_type);
...
qpnp_config_reset(struct qpnp_pon *pon, struct qpnp_pon_config *cfg)
{
	...
	printk("eztest qpnp pon config reset:%d\n",cfg->pon_type);
	switch (cfg->pon_type) {
	case PON_KPDPWR:
...
static int qpnp_pon_probe(struct spmi_device *spmi)
{
	...
	sys_reset = of_property_read_bool(spmi->dev.of_node,
						"qcom,system-reset");
	printk("eztest qpnp pon probe sys_reset:%d\n",sys_reset);
	...
	printk("eztest qpnp pon probe wm reset rc:%d\n",rc);
	if (rc) {
		dev_err(&pon->spmi->dev,
	...
	printk("eztest qpnp pon probe boot_reason:%d cold_boot:%d\n",boot_reason,cold_boot);
	if (index >= ARRAY_SIZE(qpnp_pon_reason) || index < 0) {
		...
		dev_info(&pon->spmi->dev,
			"eztest PMIC@SID%d Power-on reason: %s and '%s' boot\n",
	...
	if (index >= ARRAY_SIZE(qpnp_poff_reason) || index < 0) {
		...
		dev_info(&pon->spmi->dev,
				"eztest PMIC@SID%d: Power-off reason: %s\n",
				pon->spmi->sid,
