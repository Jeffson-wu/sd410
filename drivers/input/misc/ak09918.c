...
#define AKM_LAYOUT		0
#define AKM_AUTO		1
#define AKM_HRTIMER		0
#define AKM_RESETN		0
#define AKM_MODE_SET	1
#define AKM_POLL_INT	1000


//#define AKM_IS_MAG_DATA_ENABLED() (akm->enable_flag & (1 << MAG_DATA_FLAG))

/* POWER SUPPLY VOLTAGE RANGE */
#define AKM09911_VDD_MIN_UV	1650000
#define AKM09911_VDD_MAX_UV	1950000
...
extern uint8_t ICM_init_done;
extern int16_t ICM20600_getAccelerationX(void);
extern int16_t ICM20600_getAccelerationY(void);
extern int16_t ICM20600_getAccelerationZ(void);
extern uint8_t ICM20600_getDeviceID(void);
extern int16_t ICM20600_getTemperature(void);
extern int16_t ICM20600_getRegVal(void);
...
//	printk("RxData: ---------->i2c_addr:%x len=%02x, addr=%02x, data=%02x\n",
//		i2c->addr, length, addr, rxData[0]);
	if (ret < 0) {
...
//	akm_compass_sysfs_update_status(akm);
	...
	if (enable) {
		...
		if (akm->auto_report) {
			mode = AKM_MODE_SET;//akm_select_frequency(akm->delay[MAG_DATA_FLAG]);
			printk("ak09918 enable set---------->mode:%x\n",mode);
...
//					HRTIMER_MODE_REL);
//			else
//				queue_delayed_work(akm->work_queue, &akm->dwork, msecs_to_jiffies(AKM_POLL_INT));
				schedule_delayed_work(&akm->dwork, msecs_to_jiffies(AKM_POLL_INT));
		}
	} else {
		if (akm->auto_report) {
//			if (akm->use_hrtimer) {
		...
		unsigned int delay_msec)
{
	...
	mode = AKM_MODE_SET;//akm_select_frequency(akm->delay[MAG_DATA_FLAG]);
	printk("ak09918 delay set---------->mode:%x\n",mode);
	ret = AKECS_SetMode(akm, mode);
	if (ret < 0)
	...
	struct i2c_client *client)
{
	...
	printk("akm check device ---------> 0:%x 1:%x\n",akm->sense_info[0],akm->sense_info[1]);
	if (err < 0)
		return err;
#if 0
	...
	err = AKECS_SetMode(akm, AKM_MODE_SET/*AK09911_MODE_CONTINUOUS_100HZ*/);
	if (err < 0)
		return err;
...
static int akm_compass_parse_dt(struct device *dev,
				struct akm_compass_data *akm)
{
	akm->layout = AKM_LAYOUT;
	akm->auto_report = AKM_AUTO;
//	akm->use_hrtimer = AKM_HRTIMER;
	akm->gpio_rstn = AKM_RESETN;
#if AKM_RESETN
	if (!gpio_is_valid(akm->gpio_rstn)) {
		dev_err(dev, "gpio reset pin %d is invalid.\n",
			akm->gpio_rstn);
...
static int akm_report_data(struct akm_compass_data *akm)
{
	...
	if(ICM_init_done){
		printk("akm---->icm init:%x\n",ICM_init_done);
		printk("akm---->icm tmp:%d\n",ICM20600_getTemperature());
//		ret = ICM20600_getRegVal();
		acc_x = ICM20600_getAccelerationX();
		acc_y = ICM20600_getAccelerationY();
		acc_z = ICM20600_getAccelerationZ();
	}
	...
	printk("akm mag Data: ---------->mx:%d my:%d mz:%d\n", mag_x,mag_y,mag_z);
	input_report_abs(akm->input, ABS_X, mag_x);
	input_report_abs(akm->input, ABS_Y, mag_y);
	input_report_abs(akm->input, ABS_Z, mag_z);
	if(ICM_init_done){
		printk("akm acc Data: ---------->ax:%d ay:%d az:%d\n", acc_x,acc_y,acc_z);
		input_report_abs(akm->input, ABS_RX, acc_x);
		input_report_abs(akm->input, ABS_RY, acc_y);
		input_report_abs(akm->input, ABS_RZ, acc_z);
	}
#ifdef MATH_LAB
...
static void akm_dev_poll(struct work_struct *work)
{
	...
	if(AKM_MODE_SET == 1) ret = AKECS_SetMode(akm, AKM_MODE_SET);
...
//	init_waitqueue_head(&s_akm->open_wq);
	...
	printk("akm probe --------->1-1 lot:%d rst:%d\n",s_akm->layout,s_akm->gpio_rstn);
		if (err) {
			dev_err(&client->dev,
	...
	printk("akm probe --------->1-2 lot:%d rst:%d\n",s_akm->layout,s_akm->gpio_rstn);
	...
	printk("akm probe --------->2 lot:%d rst:%d\n",s_akm->layout,s_akm->gpio_rstn);
	/***** I2C initialization *****/
	s_akm->i2c = client;
	/* set client data */
	...
	AKECS_Reset(s_akm, AKM_RESETN);// org 1
#if 0
	/* check connection */
...
//			queue_delayed_work(s_akm->work_queue, &s_akm->dwork, msecs_to_jiffies(AKM_POLL_INT));
//			msleep(100);
			schedule_delayed_work(&s_akm->dwork, msecs_to_jiffies(AKM_POLL_INT));
//		}
//	}
#else
	err = akm_report_data(s_akm);
	if (err < 0)
...
static struct of_device_id ak09918_match_table[] = {
...
static struct i2c_driver akm_compass_driver = {
	...
	.driver = {
		.name	= AKM_I2C_NAME,
		.owner  = THIS_MODULE,
		.of_match_table = ak09918_match_table,
