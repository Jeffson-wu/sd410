...
// Hidden constructor for list4mesh, only to be used by us.
...
static hash_map_entry_t *find_bucket_entry_(list4mesh_t *hash_bucket_list4mesh,
        const void *key);
...
bool hash_map_has_key(const hash_map_t *hash_map, const void *key)
{
    ...
    list4mesh_t *hash_bucket_list4mesh = hash_map->bucket[hash_key].list4mesh;

    hash_map_entry_t *hash_map_entry = find_bucket_entry_(hash_bucket_list4mesh, key);
...
bool hash_map_set(hash_map_t *hash_map, const void *key, void *data)
{
    ...
    if (hash_map->bucket[hash_key].list4mesh == NULL) {
        hash_map->bucket[hash_key].list4mesh = list4mesh_new_internal(bucket_free_, hash_map->allocator);
        if (hash_map->bucket[hash_key].list4mesh == NULL) {
            return false;
        }
    }
    ...
    if (hash_map_entry) {
        ...
        bool rc = list4mesh_remove(hash_bucket_list4mesh, hash_map_entry);
        assert(rc == true);
    } else {
        hash_map->hash_size++;
    }
    ...
    return list4mesh_append(hash_bucket_list4mesh, hash_map_entry);
}
...
bool hash_map_erase(hash_map_t *hash_map, const void *key)
{
    ...
    return list4mesh_remove(hash_bucket_list4mesh, hash_map_entry);
}
...
void hash_map_clear(hash_map_t *hash_map)
{
    ...
    for (hash_index_t i = 0; i < hash_map->num_bucket; i++) {
        if (hash_map->bucket[i].list4mesh == NULL) {
            continue;
        }
        list4mesh_free(hash_map->bucket[i].list4mesh);
        hash_map->bucket[i].list4mesh = NULL;
    }
}
...
void hash_map_foreach(hash_map_t *hash_map, hash_map_iter_cb callback, void *context)
{
    ...
    for (hash_index_t i = 0; i < hash_map->num_bucket; ++i) {
        ...
        for (const list4mesh_node_t *iter = list4mesh_begin(hash_map->bucket[i].list4mesh);
                iter != list4mesh_end(hash_map->bucket[i].list4mesh);
                iter = list4mesh_next(iter)) {
            hash_map_entry_t *hash_map_entry = (hash_map_entry_t *)list4mesh_node(iter);
        ...
        const void *key)
{
    ...
    if (hash_bucket_list4mesh == NULL) {
        return NULL;
    }

    for (const list4mesh_node_t *iter = list4mesh_begin(hash_bucket_list4mesh);
            iter != list4mesh_end(hash_bucket_list4mesh);
            iter = list4mesh_next(iter)) {
        hash_map_entry_t *hash_map_entry = (hash_map_entry_t *)list4mesh_node(iter);
