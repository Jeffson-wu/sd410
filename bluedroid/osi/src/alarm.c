...
void alarm_set(alarm_t *alarm, period_ms_t deadline, alarm_callback_t cb, void *data) {
  ...
  ALOGE("%s alarm1.", __func__);
  pthread_mutex_lock(&monitor);

  ALOGE("%s alarm2.", __func__);
  ...
  ALOGE("%s alarm3.", __func__);
  alarm->deadline = now() + deadline;
  alarm->callback = cb;
  alarm->data = data;

  ALOGE("%s alarm4.", __func__);
...
static void reschedule(void) {
  ...
  ALOGE("%s reschedule1.", __func__);
  if (timer_set) {
    timer_delete(timer);
    timer_set = false;
  }
  ALOGE("%s reschedule2.", __func__);
  if (list_is_empty(alarms)) {
    bt_os_callouts->release_wake_lock(WAKE_LOCK_ID);
    return;
  }
  ALOGE("%s reschedule3.", __func__);
  alarm_t *next = list_front(alarms);
  if (!next) {
    bt_os_callouts->release_wake_lock(WAKE_LOCK_ID);
    return;
  }
  ALOGE("%s reschedule4.", __func__);
