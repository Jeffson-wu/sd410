...
#define LOG_TAG "btc_ble_mesh_prov"
#include <utils/Log.h>
...
void btc_ble_mesh_prov_arg_deep_copy(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    case BTC_BLE_MESH_ACT_CLIENT_MODEL_SEND: {
        ALOGE("%s, BTC_BLE_MESH_ACT_MODEL_SEND, src->model_send.length = %d", __func__, src->model_send.length);
        dst->model_send.data = src->model_send.length ? (uint8_t *)malloc(src->model_send.length) : NULL;
        ...
        ALOGE("dst->model_send.data = %s", bt_hex(dst->model_send.data, src->model_send.length));
        break;
    }
    default:
...
static void btc_ble_mesh_copy_req_data(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    case ESP_BLE_MESH_MODEL_OPERATION_EVT: {
        if (p_src_data->model_operation.ctx && p_src_data->model_operation.msg) {
            ...
            p_dest_data->model_operation.msg = p_src_data->model_operation.length ? (uint8_t *)malloc(p_src_data->model_operation.length) : NULL;
    ...
    case ESP_BLE_MESH_CLIENT_MODEL_RECV_PUBLISH_MSG_EVT: {
        if (p_src_data->client_recv_publish_msg.ctx && p_src_data->client_recv_publish_msg.msg) {
            ...
            p_dest_data->client_recv_publish_msg.msg = p_src_data->client_recv_publish_msg.length ? (uint8_t *)malloc(p_src_data->client_recv_publish_msg.length) : NULL;
        ...
        struct net_buf_simple *buf)
{
    ...
    esp_ble_mesh_model_cb_param_t mesh_param;
    ...
    if (ret != BT_STATUS_SUCCESS) {
        ALOGE("%s btc_transfer_context failed\n", __func__);
    }
    return;
}
...
static int btc_output_number_cb(bt_mesh_output_action_t act, u32_t num)
{
    ...
    btc_msg_t msg;
    bt_status_t ret;
    esp_ble_mesh_prov_cb_param_t mesh_param;
...
add_model_op:
    ...
    while (op != NULL && op->opcode != 0) {
        if (op->min_len > BLE_MESH_MAX_DATA_SIZE || op->param_cb != 0) {
            --op;
            ALOGE("%s, Invalid model operation min_len = %d, or param callbak = 0x%x, the last valid opcode = 0x%x",
...
void btc_mesh_prov_call_handler(btc_msg_t *msg)
{
    ...
    case BTC_BLE_MESH_ACT_APP_REGISTER: {
        ...
        for (int i = 0; i < (int)arg->mesh_reg.comp->element_count; i++) {
    ...
    case BTC_BLE_MESH_ACT_PROV_ENABLE:
        ALOGE("%s, BTC_BLE_MESH_ACT_PROV_ENABLE, bearers = %d", __func__, arg->mesh_prov_enable.bearers);
    ...
    case BTC_BLE_MESH_ACT_PROV_DISABLE:
        ALOGE("%s, BTC_BLE_MESH_ACT_PROV_DISABLE, bearers = %d", __func__, arg->mesh_prov_disable.bearers);
    ...
    case BTC_BLE_MESH_ACT_NODE_RESET:
        ALOGE("%s, BTC_BLE_MESH_ACT_NODE_RESET", __func__);
        bt_mesh_reset();
        break;
...
#endif /* CONFIG_BT_MESH_PROVISIONER */
    default:
        ALOGE("%s, Invalid msg->act %d", __func__, msg->act);
        break;
    }
    return;
