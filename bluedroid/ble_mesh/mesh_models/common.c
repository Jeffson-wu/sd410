...
struct net_buf_simple *bt_mesh_alloc_buf(u16_t size)
{
    ...
    buf = (struct net_buf_simple *)malloc(sizeof(struct net_buf_simple) + size);
    if (!buf) {
        ALOGE("%s: allocate buf memory fail", __func__);
        return NULL;
    }
...
int bt_mesh_copy_msg_role(bt_mesh_role_param_t *common)
{
    ...
    case PROVISIONER:
        ...
        if (!bt_mesh_is_provisioner_en()) {
            ALOGE("%s: send provisioner msg with provisioner disabled", __func__);
            return -EINVAL;
        }
...
#endif
    default:
        ALOGE("%s: role mismatch", __func__);
        return -EINVAL;
    }
...
u8_t bt_mesh_get_msg_role(struct bt_mesh_model *model, bool srv_send)
{
    ...
    if (srv_send) {
        ALOGE("%s: msg sent by a server model", __func__);
        return NODE;
    }
