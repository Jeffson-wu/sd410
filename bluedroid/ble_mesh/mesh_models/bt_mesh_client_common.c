...
#define LOG_TAG "bt_mesh_client_common"
#include <utils/Log.h>
        ...
        bool need_pub)
{
    ...
    if (!BT_MESH_ADDR_IS_UNICAST(ctx->recv_dst)) {
        ALOGE("Unexpected status message 0x%x", rsp);
                            ...
                            void *cb_data)
{
    ...
    if (bt_mesh_client_check_node_in_list(&internal->queue, ctx->addr)) {
        ...
        node = (bt_mesh_client_node_t *)malloc(sizeof(bt_mesh_client_node_t));
        if (!node) {
        ...
        if ((node->op_pending = bt_mesh_client_get_status_op(cli->op_pair, cli->op_pair_size, opcode)) == 0) {
            ALOGE("%s, Didn't find the status opcode in the op_pair list.", __func__);
            free(node);
            return -EINVAL;
        }
...
int bt_mesh_client_init(struct bt_mesh_model *model)
{
    ...
    if (!model) {
        ALOGE("Client model is NULL");
        return -EINVAL;
    }

    if (!model->op) {
        ALOGE("Client model op is NULL");
        return -EINVAL;
    }

    cli = model->user_data;
    if (!cli) {
        ALOGE("Client model user_data is NULL");
        return -EINVAL;
    }

    /* TODO: call free() when deinit function is invoked */
    data = malloc(sizeof(bt_mesh_internal_data_t));
    if (!data) {
        ALOGE("Allocate memory for client internal data fail");
        return -ENOMEM;
    }
