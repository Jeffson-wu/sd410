...
#define LOG_TAG "time_scene_client"
#include <utils/Log.h>
...
static void timeout_handler(struct k_work *work)
{
    ...
    ALOGE("Receive time scene status message timeout");
                              ...
                              struct net_buf_simple *buf)
{
    ...
    case BT_MESH_MODEL_OP_SCENE_STATUS: {
        ...
        if (buf->len != 3 && buf->len != 6) {
            ALOGE("Wrong Scene status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_scene_status));
        if (!status) {
    ...
    case BT_MESH_MODEL_OP_SCENE_REGISTER_STATUS: {
        ...
        status = malloc(sizeof(struct bt_mesh_scene_register_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    default:
        ALOGE("Not a time scene status message opcode");
        return;
    }
    ...
    if (!node) {
        ALOGE("Unexpected time scene status message 0x%x", rsp);
    } else {
        switch (node->opcode) {
...
static int time_scene_get_state(struct bt_mesh_common_param *common, void *value)
{
    ...
    if (err) {
        ALOGE("Time scene get message send failed (err %d)", err);
    }
                                ...
                                void *value, u16_t value_len, bool need_ack)
{
    ...
    if (!msg) {
        ALOGE("Time scene allocate memory fail");
        return -ENOMEM;
    }
    ...
    err = bt_mesh_client_send_msg(common->model, common->opcode, &common->ctx, msg,
                                  ...
                                  common->cb, common->cb_data);
    if (err) {
        ALOGE("Time scene set message send failed (err %d)", err);
    }
...
int bt_mesh_time_scene_client_get_state(struct bt_mesh_common_param *common, void *get, void *status)
{
    ...
    case BT_MESH_MODEL_OP_SCENE_REGISTER_GET:
        break;
    default:
        ALOGE("Not a time scene get message opcode");
        return -EINVAL;
    }
...
int bt_mesh_time_scene_client_set_state(struct bt_mesh_common_param *common, void *set, void *status)
{
    ...
    case BT_MESH_MODEL_OP_SCENE_STORE_UNACK: {
        ...
        if (!value->scene_number) {
            ALOGE("Scene store scene_number 0x0000 is prohibited");
            return -EINVAL;
        }
    ...
    case BT_MESH_MODEL_OP_SCENE_RECALL_UNACK: {
        ...
        if (!value->scene_number) {
            ALOGE("Scene recall scene_number 0x0000 is prohibited");
            return -EINVAL;
        }
        if (value->op_en) {
            if ((value->trans_time & 0x3F) > 0x3E) {
                ALOGE("Scene recall transition time is bigger than 0x3E");
                return -EINVAL;
            }
        }
    ...
    default:
        ALOGE("Not a time scene set message opcode");
        return -EINVAL;
    }
...
static int time_scene_client_init(struct bt_mesh_model *model, bool primary)
{
    ...
    if (!model) {
        ALOGE("Time scene client model is NULL");
        return -EINVAL;
    }

    client = (bt_mesh_time_scene_client_t *)model->user_data;
    if (!client) {
        ALOGE("Time scene client model user_data is NULL");
        return -EINVAL;
    }

    /* TODO: call free() when deinit function is invoked*/
    internal = malloc(sizeof(time_scene_internal_data_t));
    if (!internal) {
        ALOGE("Allocate memory for time scene internal data fail");
        return -ENOMEM;
    }
    ...
    client->model         = model;
    client->op_pair_size  = MESH_ARRAY_SIZE(time_scene_op_pair);
