...
static void timeout_handler(struct k_work *work)
{
    ...
    ALOGE("Receive light status message timeout");
                         ...
                         struct net_buf_simple *buf)
{
    ...
    case BT_MESH_MODEL_OP_LIGHT_LIGHTNESS_STATUS: {
        ...
        if (buf->len != 2 && buf->len != 5) {
            ALOGE("Wrong Light lightness status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_lightness_status));
        if (!status) {
    ...
    case BT_MESH_MODEL_OP_LIGHT_LIGHTNESS_LINEAR_STATUS: {
        ...
        status = malloc(sizeof(struct bt_mesh_light_lightness_linear_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_LIGHTNESS_LAST_STATUS: {
        ...
        if (buf->len != 2) {
            ALOGE("Wrong Light last status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_lightness_last_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_LIGHTNESS_DEFAULT_STATUS: {
        ...
        if (buf->len != 2) {
            ALOGE("Wrong Light default status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_lightness_default_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_LIGHTNESS_RANGE_STATUS: {
        ...
        status = malloc(sizeof(struct bt_mesh_light_lightness_range_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_CTL_STATUS: {
        ...
        if (buf->len != 4 && buf->len != 9) {
            ALOGE("Wrong Light CTL status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_ctl_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_CTL_TEMPERATURE_STATUS: {
        ...
        if (buf->len != 4 && buf->len != 9) {
            ALOGE("Wrong Light CTL temperature status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_ctl_temperature_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_CTL_TEMPERATURE_RANGE_STATUS: {
        ...
        if (buf->len != 5) {
            ALOGE("Wrong Light CTL temperature range status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_ctl_temperature_range_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_CTL_DEFAULT_STATUS: {
        ...
        if (buf->len != 6) {
            ALOGE("Wrong Light CTL default status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_ctl_default_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_HSL_STATUS: {
        ...
        if (buf->len != 6 && buf->len != 7) {
            ALOGE("Wrong Light HSL status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_hsl_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_HSL_TARGET_STATUS: {
        ...
        if (buf->len != 6 && buf->len != 7) {
            ALOGE("Wrong Light HSL target status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_hsl_target_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_HSL_HUE_STATUS: {
        ...
        if (buf->len != 2 && buf->len != 5) {
            ALOGE("Wrong Light HSL Hue status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_hsl_hue_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_HSL_SATURATION_STATUS: {
        ...
        if (buf->len != 2 && buf->len != 5) {
            ALOGE("Wrong Light HSL saturation status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_hsl_saturation_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_HSL_DEFAULT_STATUS: {
        ...
        if (buf->len != 6) {
            ALOGE("Wrong Light HSL default status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_hsl_default_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_HSL_RANGE_STATUS: {
        ...
        if (buf->len != 9) {
            ALOGE("Wrong Light HSL range status length %d", buf->len);
            return;
        }
        status = malloc(sizeof(struct bt_mesh_light_hsl_range_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    default:
        ALOGE("Not a lighting status message opcode");
        return;
    }
    ...
    if (!node) {
        ALOGE("Unexpected light status message 0x%x", rsp);
    } else {
        switch (node->opcode) {
...
static int light_get_state(struct bt_mesh_common_param *common, void *value)
{
    ...
    if (err) {
        ALOGE("Light get message send failed (err %d)", err);
    }
                           ...
                           void *value, u16_t value_len, bool need_ack)
{
    ...
    if (!msg) {
        ALOGE("Light set allocate memory fail");
        return -ENOMEM;
    }
    ...
    default:
        ALOGE("Not a lighting client model set message opcode");
        err = -EINVAL;
        goto end;
    }
    ...
    err = bt_mesh_client_send_msg(common->model, common->opcode, &common->ctx, msg,
                                  ...
                                  common->cb, common->cb_data);
    if (err) {
        ALOGE("Light set message send failed (err %d)", err);
    }
...
int bt_mesh_light_client_get_state(struct bt_mesh_common_param *common, void *get, void *status)
{
    ...
    case BT_MESH_MODEL_OP_LIGHT_HSL_RANGE_GET:
        break;
    default:
        ALOGE("Not a lighting get message opcode");
        return -EINVAL;
    }
...
int bt_mesh_light_client_set_state(struct bt_mesh_common_param *common, void *set, void *status)
{
    ...
    case BT_MESH_MODEL_OP_LIGHT_LIGHTNESS_SET_UNACK: {
        ...
        if (value->op_en) {
            if ((value->trans_time & 0x3F) > 0x3E) {
                ALOGE("Light lightness set transition time is bigger than 0x3E");
                return -EINVAL;
            }
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_LIGHTNESS_LINEAR_SET_UNACK: {
        ...
        if (value->op_en) {
            if ((value->trans_time & 0x3F) > 0x3E) {
                ALOGE("Light lightness linear set transition time is bigger than 0x3E");
                return -EINVAL;
            }
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_LIGHTNESS_RANGE_SET_UNACK: {
        ...
        if (value->range_min > value->range_max) {
            ALOGE("Light lightness range set range min is greater than range max");
            return -EINVAL;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_CTL_SET_UNACK: {
        ...
        if (value->op_en) {
            if ((value->trans_time & 0x3F) > 0x3E) {
                ALOGE("Light ctl set transition time is bigger than 0x3E");
                return -EINVAL;
            }
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_CTL_TEMPERATURE_SET_UNACK: {
        ...
        if (value->op_en) {
            if ((value->trans_time & 0x3F) > 0x3E) {
                ALOGE("Light ctl temperature set transition time is bigger than 0x3E");
                return -EINVAL;
            }
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_CTL_TEMPERATURE_RANGE_SET_UNACK: {
        ...
        if (value->range_min > value->range_max) {
            ALOGE("Light ctl temperature range set range min is greater than range max");
            return -EINVAL;
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_HSL_SET_UNACK: {
        ...
        if (value->op_en) {
            if ((value->trans_time & 0x3F) > 0x3E) {
                ALOGE("Light hsl set transition time is bigger than 0x3E");
                return -EINVAL;
            }
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_HSL_HUE_SET_UNACK: {
        ...
        if (value->op_en) {
            if ((value->trans_time & 0x3F) > 0x3E) {
                ALOGE("Light hsl hue set transition time is bigger than 0x3E");
                return -EINVAL;
            }
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_HSL_SATURATION_SET_UNACK: {
        ...
        if (value->op_en) {
            if ((value->trans_time & 0x3F) > 0x3E) {
                ALOGE("Light hsl saturation set transition time is bigger than 0x3E");
                return -EINVAL;
            }
        }
    ...
    case BT_MESH_MODEL_OP_LIGHT_HSL_RANGE_SET_UNACK: {
        ...
        if (value->hue_range_min > value->hue_range_max ||
            ...
            ALOGE("Light hsl range set range min is greater than range max");
            return -EINVAL;
        }
    ...
    default:
        ALOGE("Not a lighting set message opcode");
        return -EINVAL;
    }
...
static int light_client_init(struct bt_mesh_model *model, bool primary)
{
    ...
    if (!model) {
        ALOGE("Light client model is NULL");
        return -EINVAL;
    }

    client = (bt_mesh_light_client_t *)model->user_data;
    if (!client) {
        ALOGE("Light client model user_data is NULL");
        return -EINVAL;
    }

    /* TODO: call free() when deinit function is invoked*/
    internal = malloc(sizeof(light_internal_data_t));
    if (!internal) {
    ...
    client->model         = model;
    client->op_pair_size  = MESH_ARRAY_SIZE(light_op_pair);
