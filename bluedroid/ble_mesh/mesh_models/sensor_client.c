...
static void timeout_handler(struct k_work *work)
{
    ...
    ALOGE("Receive sensor status message timeout");
                          ...
                          struct net_buf_simple *buf)
{
    ...
    case BT_MESH_MODEL_OP_SENSOR_DESCRIPTOR_STATUS: {
        ...
        status = malloc(sizeof(struct bt_mesh_sensor_descriptor_status));
        if (!status) {
    ...
    case BT_MESH_MODEL_OP_SENSOR_CADENCE_STATUS: {
        ...
        status = malloc(sizeof(struct bt_mesh_sensor_cadence_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_SENSOR_SETTINGS_STATUS: {
        ...
        status = malloc(sizeof(struct bt_mesh_sensor_settings_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_SENSOR_SETTING_STATUS: {
        ...
        status = malloc(sizeof(struct bt_mesh_sensor_setting_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_SENSOR_STATUS: {
        ...
        status = malloc(sizeof(struct bt_mesh_sensor_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_SENSOR_COLUMN_STATUS: {
        ...
        status = malloc(sizeof(struct bt_mesh_sensor_column_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    case BT_MESH_MODEL_OP_SENSOR_SERIES_STATUS: {
        ...
        status = malloc(sizeof(struct bt_mesh_sensor_series_status));
        if (!status) {
            ALOGE("%s: allocate memory fail", __func__);
            return;
        }
    ...
    default:
        ALOGE("Not a sensor status message opcode");
        return;
    }
    ...
    if (!node) {
        ALOGE("Unexpected sensor status message 0x%x", rsp);
    } else {
        switch (node->opcode) {
                            ...
                            void *value, u16_t value_len, bool need_ack)
{
    ...
    if (!msg) {
        ALOGE("Sensor allocate memory fail");
        return -ENOMEM;
    }
    ...
    default:
        ALOGE("Not a sensor client model message opcode");
        err = -EINVAL;
        goto end;
    }
    ...
    if (err) {
        ALOGE("Sensor message send failed (err %d)", err);
    }
...
int bt_mesh_sensor_client_get_state(struct bt_mesh_common_param *common, void *get, void *status)
{
    ...
    case BT_MESH_MODEL_OP_SENSOR_COLUMN_GET: {
        ...
        if (!value->raw_value_x) {
            ALOGE("Sensor column get is NULL");
            return -EINVAL;
        }
    ...
    case BT_MESH_MODEL_OP_SENSOR_SERIES_GET: {
        ...
        if (value->op_en) {
            if (!value->raw_value_x1 || !value->raw_value_x2) {
                ALOGE("Sensor series get parameter is NULL");
                return -EINVAL;
            }
        }
        if (value->op_en) {
    ...
    default:
        ALOGE("Not a sensor get message opcode");
        return -EINVAL;
    }
...
int bt_mesh_sensor_client_set_state(struct bt_mesh_common_param *common, void *set, void *status)
{
    ...
    case BT_MESH_MODEL_OP_SENSOR_CADENCE_SET_UNACK: {
        ...
        if (!value->status_trigger_delta_down || !value->status_trigger_delta_up ||
            ...
            ALOGE("Sensor cadence set parameter is NULL");
            return -EINVAL;
        }
    ...
    case BT_MESH_MODEL_OP_SENSOR_SETTING_SET_UNACK: {
        ...
        if (!value->sensor_setting_raw) {
            ALOGE("Sensor_setting_raw is NULL");
            return -EINVAL;
        }
    ...
    default:
        ALOGE("Not a sensor set message opcode");
        return -EINVAL;
    }
...
int bt_mesh_sensor_cli_init(struct bt_mesh_model *model, bool primary)
{
    ...
    if (!model) {
        ALOGE("Sensor client model is NULL");
        return -EINVAL;
    }

    client = (bt_mesh_sensor_client_t *)model->user_data;
    if (!client) {
        ALOGE("Sensor client model user_data is NULL");
        return -EINVAL;
    }

    /* TODO: call free() when deinit function is invoked*/
    internal = malloc(sizeof(sensor_internal_data_t));
    if (!internal) {
        ALOGE("Allocate memory for sensor internal data fail");
        return -ENOMEM;
    }
    ...
    client->model         = model;
    client->op_pair_size  = MESH_ARRAY_SIZE(sensor_op_pair);
