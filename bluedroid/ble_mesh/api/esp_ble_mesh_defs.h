...
/*!< Defines of esp_ble_mesh_opcode_t */
/*!< The following opcodes are not supported currently */
#define ESP_BLE_MESH_MODEL_OP_NET_KEY_UPDATE                        OP_NET_KEY_UPDATE
#define ESP_BLE_MESH_MODEL_OP_NET_KEY_DELETE                        OP_NET_KEY_DEL
#define ESP_BLE_MESH_MODEL_OP_NET_KEY_GET                           OP_NET_KEY_GET
#define ESP_BLE_MESH_MODEL_OP_APP_KEY_UPDATE                        OP_APP_KEY_UPDATE
#define ESP_BLE_MESH_MODEL_OP_APP_KEY_DELETE                        OP_APP_KEY_DEL
#define ESP_BLE_MESH_MODEL_OP_APP_KEY_GET                           OP_APP_KEY_GET
#define ESP_BLE_MESH_MODEL_OP_NODE_IDENTITY_GET                     OP_NODE_IDENTITY_GET
#define ESP_BLE_MESH_MODEL_OP_NODE_IDENTITY_SET                     OP_NODE_IDENTITY_SET
#define ESP_BLE_MESH_MODEL_OP_KEY_REFRESH_PHASE_GET                 OP_KRP_GET
#define ESP_BLE_MESH_MODEL_OP_KEY_REFRESH_PHASE_SET                 OP_KRP_SET
#define ESP_BLE_MESH_MODEL_OP_MODEL_PUB_VIRTUAL_ADDR_SET            OP_MOD_PUB_VA_SET
#define ESP_BLE_MESH_MODEL_OP_MODEL_SUB_DELETE_ALL                  OP_MOD_SUB_DEL_ALL
#define ESP_BLE_MESH_MODEL_OP_SIG_MODEL_SUB_GET                     OP_MOD_SUB_GET
#define ESP_BLE_MESH_MODEL_OP_VENDOR_MODEL_SUB_GET                  OP_MOD_SUB_GET_VND
#define ESP_BLE_MESH_MODEL_OP_SIG_MODEL_APP_GET                     OP_SIG_MOD_APP_GET
#define ESP_BLE_MESH_MODEL_OP_VENDOR_MODEL_APP_GET                  OP_VND_MOD_APP_GET
#define ESP_BLE_MESH_MODEL_OP_MODEL_APP_UNBIND                      OP_MOD_APP_UNBIND
#define ESP_BLE_MESH_MODEL_OP_LOW_POWER_NODE_POLLTIMEOUT_GET        OP_LPN_TIMEOUT_GET
#define ESP_BLE_MESH_MODEL_OP_NETWORK_TRANSMIT_GET                  OP_NET_TRANSMIT_GET
#define ESP_BLE_MESH_MODEL_OP_NETWORK_TRANSMIT_SET                  OP_NET_TRANSMIT_SET
...
#define ESP_BLE_MESH_MODEL_OP_LOW_POWER_NODE_POLLTIMEOUT_STATUS     OP_LPN_TIMEOUT_STATUS
...
/* esp_ble_mesh_cb_t has no need to be initialized by the users (set with 0 and will be initialized internally) */
...
{                                                       \
    .location         = (_loc),                         \
    .sig_model_count  = MESH_ARRAY_SIZE(_mods),              \
    .sig_models       = (_mods),                        \
    .vnd_model_count  = MESH_ARRAY_SIZE(_vnd_mods),          \
...
typedef struct {
     ...
     *
     *  @param remote_pub_key: Pointer to the public key of the device.
     *
     *  @return 0 on success or negative error code otherwise
     */
    int            (*prov_pub_key_oob_cb)(u8_t *remote_pub_key);
...
typedef struct {
    esp_ble_mesh_opcode_t opcode;   /*!< The Message opcode */
    ...
    int32_t msg_timeout;            /*!< Timeout value (ms) used to get response to the sent message */
    /*!< Note: if using default timeout value in menuconfig, make sure to set it to 0 */
...
typedef enum {
    ESP_BLE_MESH_PROV_REGISTER_COMP_EVT,                        /*!< Initialize provisioning capabilities and properties completion event */
    ...
    ESP_BLE_MESH_NODE_PROV_INPUT_NUM_COMP_EVT,                  /*!< Node input number completion event */
    ESP_BLE_MESH_NODE_PROV_INPUT_STR_COMP_EVT,                  /*!< Node input string completion event */
...
typedef enum {
    ...
    ESP_BLE_MESH_CLIENT_MODEL_SEND_TIMEOUT_EVT,     /*!< Timeout event of the user-defined client models failed to receive response from peer server models */
    ESP_BLE_MESH_MODEL_PUBLISH_UPDATE_EVT,          /*!< When a model is configured to publish messages periodically, this event will come during every publish period */
...
typedef struct {
    ...
    uint32_t publish_status;                        /*!< This variable is reserved for BLE Mesh Stack, does not require initializing in the application layer */
