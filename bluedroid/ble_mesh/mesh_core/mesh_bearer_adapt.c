 ...
 * Additional Copyright (c) 2019 Borderless Incoperated.
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#define LOG_TAG "mesh_bearer_adapter"
#include <utils/Log.h>
...
static void scan_result_callback(tBTA_DM_SEARCH_EVT event, tBTA_DM_SEARCH *p_data)
{
    ...
    ALOGE("addr_type = %d", p_data->inq_res.ble_addr_type);
    ALOGE("adv_data: %s", bt_hex(p_data->inq_res.p_eir, p_data->inq_res.adv_data_len));
    if (event == BTA_DM_INQ_RES_EVT) {
        ...
        buf = (struct net_buf_simple *) malloc (sizeof(struct net_buf_simple) + p_data->inq_res.adv_data_len);
        buf->data = buf->__buf;
        ...
        ALOGE("%s, Scan completed. Num Resp %d\n", __func__, p_data->inq_cmpl.num_resps);
    } else {
        ALOGE("%s, unexpected events 0x%x\n", __func__, p_data->inq_cmpl.num_resps);
    }
}
...
static void mesh_bta_gatts_cb(tBTA_GATTS_EVT event, tBTA_GATTS *p_data)
{
    ...
    case BTA_GATTS_READ_EVT: {
        ...
        uint16_t len = 0;
        tBTA_GATTS_RSP rsp;
        ALOGE("%s, read: handle = %d", __func__, p_data->req_data.p_data->read_req.handle);
        ...
        if (mesh_attr != NULL && mesh_attr->read != NULL) {
            if ((len = mesh_attr->read(&mesh_conn[index], mesh_attr, buf, 100,
                ...
                ALOGE("%s, send mesh read rsp, handle = %x", __func__, mesh_attr->handle);
            } else {
                ALOGE("%s, read perm Invalid", __func__);
            }
        }
        break;
    }
    case BTA_GATTS_WRITE_EVT: {
        //void *buf = NULL;
        uint16_t len = 0;
        ALOGE("%s, write: handle = %d, len = %d, data = %s", __func__, p_data->req_data.p_data->write_req.handle,
        ...
        if (mesh_attr != NULL && mesh_attr->write != NULL) {
            if ((len = mesh_attr->write(&mesh_conn[index], mesh_attr,
                ...
                if (p_data->req_data.p_data->write_req.need_rsp) {
                    ...
                    ALOGE("%s, send mesh write rsp, handle = %x", __func__, mesh_attr->handle);
                }
            }
        }
        break;
    ...
    case BTA_GATTS_CREATE_EVT: {
        ...
        ALOGE("%s, svc_handle = %d, future_mesh = %p", __func__, svc_handle, future_mesh);
        if (future_mesh != NULL) {
...
static void mesh_bta_gattc_cb(tBTA_GATTC_EVT event, tBTA_GATTC *p_data)
{
    ...
    case BTA_GATTC_REG_EVT:
        if (p_data->reg_oper.status == BTA_GATT_OK) {
            ...
            if (p_data->reg_oper.app_uuid.len == LEN_UUID_128 &&
                ...
                ALOGE("mesh_gattc_if is %d", mesh_gattc_if);
            }
        }
        break;
    ...
    case BTA_GATTC_CFG_MTU_EVT:
        if (p_data->cfg_mtu.status == BTA_GATT_OK) {
            ALOGE("BTA_GATTC_CFG_MTU_EVT, cfg_mtu is %d", p_data->cfg_mtu.mtu);
    ...
    case BTA_GATTC_SEARCH_RES_EVT: {
        ALOGE("BTA_GATTC_SEARCH_RES_EVT");
    ...
    case BTA_GATTC_SEARCH_CMPL_EVT: {
        if (p_data->search_cmpl.status == BTA_GATT_OK) {
            ALOGE("BTA_GATTC_SEARCH_CMPL_EVT");
            ...
            if (i == CONFIG_BT_MESH_PBG_SAME_TIME) {
                ALOGE("BTA_GATTC_SEARCH_CMPL_EVT: conn_id not found");
                return;
            }
    ...
    case BTA_GATTC_WRITE_DESCR_EVT:
        if (p_data->write.status == BTA_GATT_OK) {
            ALOGE("BTA_GATTC_WRITE_DESCR_EVT");
            ...
            if (i == CONFIG_BT_MESH_PBG_SAME_TIME) {
                ALOGE("BTA_GATTC_WRITE_DESCR_EVT: conn_id not found");
                return;
            }
            ...
            if (gattc_info[i].ccc_handle != p_data->write.handle) {
                ALOGE("BTA_GATTC_WRITE_DESCR_EVT: ccc_handle not match");
                bt_gattc_disconnect(conn);
                return;
            }
    ...
    case BTA_GATTC_NOTIF_EVT: {
        ...
        if (gattc_info[i].srvc_uuid == BT_UUID_MESH_PROV_VAL) {
            if (mesh_prov_conn_cb != NULL && mesh_prov_conn_cb->prov_notify != NULL) {
                ...
                if (len < 0) {
                    ALOGE("Receive prov_notify fail");
                    bt_gattc_disconnect(conn);
                    return;
                }
            }
            ...
            if (mesh_prov_conn_cb != NULL && mesh_prov_conn_cb->proxy_notify != NULL) {
                len = mesh_prov_conn_cb->proxy_notify(&gattc_info[i].conn,
                                                      p_data->notify.value, p_data->notify.len);
                if (len < 0) {
                    ALOGE("Receive proxy_notify fail");
                    bt_gattc_disconnect(conn);
                    return;
                }
            }
    ...
    case BTA_GATTC_CLOSE_EVT:
        ALOGE("BTA_GATTC_CLOSE_EVT");
        break;

    case BTA_GATTC_CONNECT_EVT: {
        ALOGE("BTA_GATTC_CONNECT_EVT");

        if (mesh_gattc_if != p_data->connect.client_if) {
            ALOGE("mesh_gattc_if & connect_if don't match");
            return;
        }
    ...
    case BTA_GATTC_DISCONNECT_EVT: {
        ALOGE("BTA_GATTC_DISCONNECT_EVT");

        if (mesh_gattc_if != p_data->disconnect.client_if) {
            ALOGE("mesh_gattc_if & disconnect_if don't match");
            return;
        } else {
            /* Increment pbg_count */
...
size_t bt_gatt_attr_read(struct bt_conn *conn, const struct bt_gatt_attr *attr,
                          ...
                          const void *value, u16_t value_len)
{
    ...
    ALOGE("handle 0x%04x offset %u length %u", attr->handle, offset,
           len);

    memcpy(buf, (char*)value + offset, len);
...
size_t bt_gatt_attr_read_included(struct bt_conn *conn,
...
size_t bt_gatt_attr_read_service(struct bt_conn *conn,
                                  const struct bt_gatt_attr *attr,
                                  void *buf, u16_t len, u16_t offset)
{
...
size_t bt_gatt_attr_read_chrc(struct bt_conn *conn,
                               ...
                               u16_t len, u16_t offset)
{
    ...
    if (!next) {
        ALOGE("No value for characteristic at 0x%04x", attr->handle);
...
static void bta_uuid_to_mesh_uuid(tBT_UUID *bta_uuid, const struct bt_uuid  *uuid)
{
    ...
    if (uuid->type == BT_UUID_TYPE_16) {
        ...
        ALOGE("%s, Invalid mesh uuid type = %d", __func__, uuid->type);
    }
...
populate:
    ...
    for (; attrs && count; attrs++, count--) {
        if (!attrs->handle) {
            ...
            ALOGE("Unable to register handle 0x%04x",
                   attrs->handle);
            return -EINVAL;
        }

        ALOGE("attr %p handle 0x%04x uuid %s perm 0x%02x",
        ...
        if (svc->attrs[i].uuid->type == BT_UUID_TYPE_16) {
            ...
            case BT_UUID_GATT_PRIMARY_VAL: {
                ...
                if (future_await(future_mesh) == FUTURE_FAIL) {
                    ALOGE("add primary service failed.");
                    return 1;
                }
                svc->attrs[i].handle = svc_handle;
                ALOGE("############## create service ############");
                ALOGE("add pri service: svc_uuid = %x, perm = %d, svc_handle = %d", bta_uuid.uu.uuid16, svc->attrs[i].perm, svc_handle);
                break;
            }
            case BT_UUID_GATT_SECONDARY_VAL: {
                ...
                if (future_await(future_mesh) == FUTURE_FAIL) {
                    ALOGE("add secondary service failed.");
                    return 1;
                }
                svc->attrs[i].handle = svc_handle;
                ALOGE("add sec service: svc_uuid = %x, perm = %d, svc_handle = %d", bta_uuid.uu.uuid16, svc->attrs[i].perm, svc_handle);
                break;
            }
            ...
            case BT_UUID_GATT_CHRC_VAL: {
                ...
                BTA_GATTS_AddCharacteristic(svc_handle, &bta_uuid, mesh_perm_to_bta_perm(svc->attrs[i + 1].perm), gatt_chrc->properties);
                if (future_await(future_mesh) == FUTURE_FAIL) {
                    ALOGE("Add characristic failed.");
                    return 1;
                }
                ...
                ALOGE("add char: char_uuid = %x, char_handle = %d, perm = %d, char_pro = %d", BT_UUID_16(gatt_chrc->uuid)->val, char_handle, svc->attrs[i + 1].perm, gatt_chrc->properties);
                break;
            }
            ...
            case BT_UUID_ES_TRIGGER_SETTING_VAL: {
                ...
                BTA_GATTS_AddCharDescriptor(svc_handle, mesh_perm_to_bta_perm(svc->attrs[i].perm), &bta_uuid);
                if (future_await(future_mesh) == FUTURE_FAIL) {
                    ALOGE("add primary service failed.");
                    return 1;
                }
                svc->attrs[i].handle = char_handle;
                ALOGE("add descr: descr_uuid = %x, perm= %d, descr_handle = %d", BT_UUID_16(svc->attrs[i].uuid)->val, svc->attrs[i].perm, char_handle);
                break;
            }
            default:
                break;
...
struct bt_conn *bt_conn_ref(struct bt_conn *conn)
{
    ...
    ALOGE("handle %u ref %u", conn->handle, atomic_get(&conn->ref));
...
int bt_rand(void *buf, size_t len)
{
    ...
    for (int i = 0; i < (int)(len / sizeof(uint32_t)); i++) {
        ...
        memcpy((char*)buf + i * sizeof(uint32_t), &rand, sizeof(uint32_t));
    }

    ALOGE("%s, rand: %s", __func__, bt_hex(buf, len));
    return 0;
}
...
{
#if CONFIG_MBEDTLS_HARDWARE_AES
    ALOGE("key %s plaintext %s", bt_hex(key, 16), bt_hex(plaintext, 16));

    ecb_encrypt(key, plaintext, enc_data, NULL);

    ALOGE("enc_data %s", bt_hex(enc_data, 16));
    return 0;
...
#endif
    ALOGE("gen the pub_key:%s", bt_hex(pub_key, sizeof(pub_key)));
    return pub_key;
}
...
static void bt_mesh_rand_cb(tBTM_RAND_ENC *p)
{
    ...
    ALOGE("%s, Generate the rand: %s", __func__, bt_hex(p->param_buf, p->param_len));
...
int bt_dh_key_gen(const u8_t remote_pk[64], bt_dh_key_cb_t cb)
{
    ...
    ALOGE("private key = %s", bt_hex(mesh_private_key, BT_OCTET32_LEN));

    //ALOGE("remote public key = %s", bt_hex(remote_pk, 64));
...
#endif
    ALOGE("peer_publ_key.x = %s", bt_hex(peer_publ_key.x, BT_OCTET32_LEN));
    ALOGE("peer_publ_key.y = %s", bt_hex(peer_publ_key.y, BT_OCTET32_LEN));
...
#endif
    ALOGE("=== x = %s", bt_hex(new_publ_key.x, 32));
    ALOGE("===y = %s", bt_hex(new_publ_key.y, 32));
...
{
    if (!svc) {
        ALOGE("%s, svc should not be NULL", __func__);
        return -EINVAL;
    }

    ALOGE("Stop service:%d\n", svc->attrs[0].handle);
