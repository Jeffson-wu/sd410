...
#define ALOGE_ENABLED IS_ENABLED(CONFIG_BT_MESH_DEBUG_CRYPTO)
               ...
               u8_t net_id[1], u8_t enc_key[16], u8_t priv_key[16])
{
    ...
    ALOGE("n %s", bt_hex(n, 16));
    ALOGE("p %s", bt_hex(p, p_len));
    ...
    err = bt_mesh_aes_cmac(t, sg, MESH_ARRAY_SIZE(sg), out);
    if (err) {
        return err;
    }
    ...
    ALOGE("NID 0x%02x enc_key %s", net_id[0], bt_hex(enc_key, 16));
    ALOGE("priv_key %s", bt_hex(priv_key, 16));
                               ...
                               u8_t *out_msg, size_t mic_size)
{
    ...
    ALOGE("key %s", bt_hex(key, 16));
    ALOGE("nonce %s", bt_hex(nonce, 13));
    ALOGE("msg (len %u) %s", msg_len, bt_hex(msg, msg_len));
    ALOGE("aad_len %u mic_size %u", aad_len, mic_size);
                          ...
                          const u8_t privacy_key[16])
{
    ...
    ALOGE("IVIndex %u, PrivacyKey %s", iv_index, bt_hex(privacy_key, 16));

    sys_put_be32(iv_index, &priv_rand[5]);
    memcpy(&priv_rand[9], &pdu[7], 7);

    ALOGE("PrivacyRandom %s", bt_hex(priv_rand, 16));
                        ...
                        u32_t iv_index, bool proxy)
{
    ...
    ALOGE("IVIndex %u EncKey %s mic_len %u", iv_index, bt_hex(key, 16),
           mic_len);
    ALOGE("PDU (len %u) %s", buf->len, bt_hex(buf->data, buf->len));
                        ...
                        u32_t iv_index, bool proxy)
{
    ...
    ALOGE("PDU (%u bytes) %s", buf->len, bt_hex(buf->data, buf->len));
    ALOGE("iv_index %u, key %s mic_len %u", iv_index, bt_hex(key, 16),
           mic_len);
                        ...
                        u16_t src, u16_t dst, u32_t seq_num, u32_t iv_index)
{
    ...
    ALOGE("AppKey %s", bt_hex(key, 16));
    ALOGE("dev_key %u src 0x%04x dst 0x%04x", dev_key, src, dst);
    ALOGE("seq_num 0x%08x iv_index 0x%08x", seq_num, iv_index);
    ALOGE("Clear: %s", bt_hex(buf->data, buf->len));
    ...
    if (!err) {
        ...
        ALOGE("Encr: %s", bt_hex(buf->data, buf->len));
    }
                        ...
                        u32_t iv_index)
{
    ...
    ALOGE("EncData (len %u) %s", buf->len, bt_hex(buf->data, buf->len));
                      ...
                      const u8_t auth[16], u8_t conf[16])
{
    ...
    ALOGE("ConfirmationKey %s", bt_hex(conf_key, 16));
    ALOGE("RandomDevice %s", bt_hex(rand, 16));
    ALOGE("AuthValue %s", bt_hex(auth, 16));

    return bt_mesh_aes_cmac(conf_key, sg, MESH_ARRAY_SIZE(sg), conf);
}
                        ...
                        u8_t auth[8])
{
    ...
    ALOGE("BeaconKey %s", bt_hex(beacon_key, 16));
    ALOGE("NetId %s", bt_hex(net_id, 8));
    ALOGE("IV Index 0x%08x", iv_index);

    msg[0] = flags;
    memcpy(&msg[1], net_id, 8);
    sys_put_be32(iv_index, &msg[9]);

    ALOGE("BeaconMsg %s", bt_hex(msg, sizeof(msg)));
