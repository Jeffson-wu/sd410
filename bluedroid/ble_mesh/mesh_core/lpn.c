...
static inline void group_zero(atomic_t *target)
{
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(bt_mesh.lpn.added); i++) {
...
static void lpn_group_add(u16_t group)
{
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(lpn->groups); i++) {
...
static bool sub_update(u8_t op)
{
    ...
    for (i = 0, g = 0; i < MESH_ARRAY_SIZE(lpn->groups); i++) {
        ...
        if (g == MESH_ARRAY_SIZE(req.addr_list)) {
            break;
        }
    }
