...
#define ALOGE_ENABLED IS_ENABLED(CONFIG_BT_MESH_DEBUG_BEACON)
                           ...
                           struct net_buf_simple *buf)
{
    ...
    ALOGE("net_idx 0x%04x flags 0x%02x NetID %s", sub->net_idx,
           flags, bt_hex(keys->net_id, 8));
    ALOGE("IV Index 0x%08x Auth %s", bt_mesh.iv_index,
           bt_hex(sub->auth, 8));
}
...
static int secure_beacon_send(void)
{
    ...
    for (i = 0; i < (int)MESH_ARRAY_SIZE(bt_mesh.sub); i++) {
        ...
        if (time_diff < K_SECONDS(600) &&
                time_diff < (unsigned int)BEACON_THRESHOLD(sub)) {
            continue;
        }
        ...
        if (!buf) {
            ALOGE("Unable to allocate beacon buffer");
            return -ENOBUFS;
        }
...
static int unprovisioned_beacon_send(void)
{
    ...
    adv = BT_MESH_ADV(buf);
    ALOGE("%s, adv->type = %d, adv->count = %d, adv->adv_int = %d", __func__,
...
static void secure_beacon_recv(struct net_buf_simple *buf)
{
    ...
    if (buf->len < 21) {
        ALOGE("Too short secure beacon (len %u)", buf->len);
        return;
    }
    ...
    if (bt_mesh_subnet_get(BT_MESH_KEY_PRIMARY) &&
        ...
        ALOGE("Ignoring secure beacon on non-primary subnet");
        goto update_stats;
    }

    ALOGE("net_idx 0x%04x iv_index 0x%08x, current iv_index 0x%08x",
...
void bt_mesh_beacon_recv(struct net_buf_simple *buf)
{
    ...
    case BEACON_TYPE_UNPROVISIONED:
        ALOGE("Ignoring unprovisioned device beacon");
        break;
    case BEACON_TYPE_SECURE:
        secure_beacon_recv(buf);
        break;
    default:
        ALOGE("Unknown beacon type 0x%02x", type);
        break;
    }
}
