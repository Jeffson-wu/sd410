...
int provisioner_prov_reset_all_nodes(void)
{
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(node); i++) {
...
static int provisioner_dev_find(const bt_addr_le_t *addr, const u8_t uuid[16], int *index)
{
     ...
     */
    if (uuid) {
        for (i = 0; i < MESH_ARRAY_SIZE(unprov_dev); i++) {
    ...
    if (addr && comp && (addr->type <= BLE_ADDR_RANDOM)) {
        for (j = 0; j < MESH_ARRAY_SIZE(unprov_dev); j++) {
...
start:
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(link); i++) {
        ...
        const u8_t *match, bool prov_flag)
{
    ...
    if (length && !prov_ctx.match_value) {
        prov_ctx.match_value = malloc(16);
...
static void free_segments(int id)
{
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(link[id].tx.buf); i++) {
...
static void send_reliable(int id)
{
    ...
    for (int i = 0; i < MESH_ARRAY_SIZE(link[id].tx.buf); i++) {
        struct net_buf *buf = link[id].tx.buf[i];
        ...
        if (i + 1 < MESH_ARRAY_SIZE(link[id].tx.buf) && link[id].tx.buf[i + 1]) {
...
static int prov_send_adv(struct net_buf_simple *msg)
{
    ...
    for (seg_id = 1; msg->len > 0; seg_id++) {
        if (seg_id >= MESH_ARRAY_SIZE(link[i].tx.buf)) {
...
static int prov_auth(u8_t method, u8_t action, u8_t size)
{
    ...
    link[i].auth = (u8_t *)malloc(PROV_AUTH_VAL_SIZE);
    if (!link[i].auth) {
...
static void send_confirm(void)
{
    ...
    link[i].conf_salt = (u8_t *)malloc(PROV_CONF_SALT_SIZE);
    if (!link[i].conf_salt) {
        BT_ERR("Allocate conf_salt memory fail");
        goto fail;
    }

    link[i].conf_key = (u8_t *)malloc(PROV_CONF_KEY_SIZE);
    if (!link[i].conf_key) {
...
static void prov_dh_key_cb(const u8_t key[32])
{
    ...
    link[i].dhkey = (u8_t *)malloc(PROV_DH_KEY_SIZE);
    if (!link[i].dhkey) {
...
static void prov_confirm(const u8_t *data)
{
    ...
    link[i].conf = (u8_t *)malloc(PROV_CONFIRM_SIZE);
    if (!link[i].conf) {
...
static void prov_random(const u8_t *data)
{
     ...
     *  should be done:
     *  1. malloc memory for prov_salt
     *  2. calculate prov_salt
     *  3. prepare provisioning data and send
     */
    link[i].prov_salt = (u8_t *)malloc(PROV_PROV_SALT_SIZE);
    if (!link[i].prov_salt) {
...
static void link_ack(struct prov_rx *rx, struct net_buf_simple *buf)
{
    ...
    link[i].conf_inputs = (u8_t *)malloc(PROV_CONF_INPUTS_SIZE);
...
int provisioner_pb_gatt_open(struct bt_conn *conn, u8_t *addr)
{
    ...
    link[id].conf_inputs = (u8_t *)malloc(PROV_CONF_INPUTS_SIZE);
