                                  ...
                                  struct net_buf_simple *msg)
{
    ...
    ALOGE("Company ID 0x%04x", company_id);
    ...
    if (srv->cb && srv->cb->fault_get_reg) {
        ...
        err = srv->cb->fault_get_reg(mod, company_id, test_id,
                                     ...
                                     &fault_count);
        if (err) {
            ALOGE("Failed to get faults (err %d)", err);
            *test_id = HEALTH_TEST_STANDARD;
        } else {
            net_buf_simple_add(msg, fault_count);
        }
    } else {
        ALOGE("No callback for getting faults");
        *test_id = HEALTH_TEST_STANDARD;
    }
}
                             ...
                             struct net_buf_simple *buf)
{
    ...
    ALOGE("company_id 0x%04x", company_id);

    health_get_registered(model, company_id, sdu);

    if (bt_mesh_model_send(model, ctx, sdu, NULL, NULL)) {
        ALOGE("Unable to send Health Current Status response");
    }
}
                                    ...
                                    struct net_buf_simple *buf)
{
    ...
    ALOGE("test 0x%02x company 0x%04x", test_id, company_id);
                              ...
                              struct net_buf_simple *buf)
{
    ...
    if (srv->cb && srv->cb->fault_test) {
        ...
        if (err) {
            ALOGE("Running fault test failed with err %d", err);
            return;
        }
    }
                                  ...
                                  struct bt_mesh_msg_ctx *ctx)
{
    ...
    ALOGE("%u second%s", time, (time == 1) ? "" : "s");
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Attention Status");
    }
}
                                      ...
                                      struct bt_mesh_msg_ctx *ctx)
{
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Health Period Status");
    }
}
                                    ...
                                    struct net_buf_simple *buf)
{
    ...
    if (period > 15) {
        ALOGE("Prohibited period value %u", period);
        return;
    }

    ALOGE("period %u", period);
...
int bt_mesh_health_srv_init(struct bt_mesh_model *model, bool primary)
{
    ...
    if (!srv) {
        ...
        ALOGE("No Health Server context provided");
        return -EINVAL;
    }

    if (!model->pub) {
        ALOGE("Health Server has no publication support");
        return -EINVAL;
    }
...
void bt_mesh_attention(struct bt_mesh_model *model, u8_t time)
{
    ...
    if (!model) {
        srv = health_srv;
        if (!srv) {
            ALOGE("No Health Server available");
            return;
        }
