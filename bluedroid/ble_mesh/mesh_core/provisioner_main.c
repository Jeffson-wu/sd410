...
static int provisioner_index_check(int node_index)
{
    ...
    if (node_index >= MESH_ARRAY_SIZE(mesh_nodes)) {
                               ...
                               u8_t flags, u32_t iv_index, const u8_t dev_key[16])
{
    ...
    if (mesh_node_count >= MESH_ARRAY_SIZE(mesh_nodes)) {
        BT_ERR("Provisioner mesh node queue is full");
        return -ENOMEM;
    }

    if (node_index >= MESH_ARRAY_SIZE(mesh_nodes) || !uuid || !dev_key) {
        BT_ERR("Argument is invalid");
        return -EINVAL;
    }

    node = malloc(sizeof(struct bt_mesh_node_t));
    if (!node) {
...
int provisioner_node_reset(int node_index)
{
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(bt_mesh.rpl); i++) {
        rpl = &bt_mesh.rpl[i];
...
int provisioner_upper_reset_all_nodes(void)
{
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(mesh_nodes); i++) {
...
int provisioner_upper_init(void)
{
    ...
    sub = malloc(sizeof(struct bt_mesh_subnet));
    if (!sub) {
...
const u8_t *provisioner_net_key_get(u16_t net_idx)
{
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(bt_mesh.p_sub); i++) {
        sub = bt_mesh.p_sub[i];
...
struct bt_mesh_app_key *provisioner_app_key_find(u16_t app_idx)
{
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(bt_mesh.p_app_keys); i++) {
...
static int bt_mesh_provisioner_set_kr_flag(u16_t net_idx, bool kr_flag)
{
    ...
    if (i == MESH_ARRAY_SIZE(bt_mesh.p_sub)) {
        return -ENODEV;
    }
...
int bt_mesh_provisioner_store_node_info(struct bt_mesh_node_t *node_info)
{
    ...
    for (i = CONFIG_BT_MESH_MAX_PROV_NODES; i < MESH_ARRAY_SIZE(mesh_nodes); i++) {
        node = mesh_nodes[i];
        if (!node) {
            node = malloc(sizeof(struct bt_mesh_node_t));
            if (!node) {
...
{
    return MESH_ARRAY_SIZE(bt_mesh.p_sub);
}

u32_t bt_mesh_provisioner_get_app_key_count(void)
{
    return MESH_ARRAY_SIZE(bt_mesh.p_app_keys);
}
...
int bt_mesh_provisioner_local_app_key_add(const u8_t app_key[16], u16_t net_idx, u16_t *app_idx)
{
    ...
    key = malloc(sizeof(struct bt_mesh_app_key));
    if (!key) {
        ...
        u16_t cid, u16_t app_idx)
{
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(model->keys); i++) {
...
int bt_mesh_temp_prov_app_idx_set(const u8_t app_key[16], u16_t net_idx, u16_t *app_idx, u8_t *status)
{
    ...
    while (1) {
        ...
        if (i == MESH_ARRAY_SIZE(bt_mesh.p_app_keys)) {
            break;
        }
    }
