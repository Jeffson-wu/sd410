...
static void timeout_handler(struct k_work *work)
{
    ...
    ALOGE("Receive health status message timeout");
                                 ...
                                 void *status, size_t len)
{
    ...
    if (!data) {
        ALOGE("%s: health client internal_data is NULL", __func__);
        return;
    }
    ...
    if (!node) {
        ALOGE("Unexpected health status message 0x%x", ctx->recv_op);
    } else {
        switch (node->opcode) {
                                ...
                                struct net_buf_simple *buf)
{
    ...
    if (!status.fault_array) {
        ALOGE("%s: allocate memory for fault_array fail", __func__);
        return;
    }
                                  ...
                                  struct net_buf_simple *buf)
{
    ...
    ALOGE("Test ID 0x%02x Company ID 0x%04x Fault Count %u",
           test_id, cid, buf->len);
}
...
int bt_mesh_health_cli_set(struct bt_mesh_model *model)
{
    if (!model || !model->user_data) {
        ALOGE("No Health Client context for given model");
        return -EINVAL;
    }
...
int bt_mesh_health_cli_init(struct bt_mesh_model *model, bool primary)
{
    ...
    if (!model) {
        ALOGE("Health Client model is NULL");
        return -EINVAL;
    }

    client = (bt_mesh_health_client_t *)model->user_data;
    if (!client) {
        ALOGE("No Health Client context provided");
        return -EINVAL;
    }

    /* TODO: call free() when deinit function is invoked*/
    internal = malloc(sizeof(health_internal_data_t));
    if (!internal) {
        ALOGE("Allocate memory for Health Client internal data fail");
        return -ENOMEM;
    }
    ...
    client->model         = model;
    client->op_pair_size  = MESH_ARRAY_SIZE(health_op_pair);
