...
#define ALOGE_ENABLED IS_ENABLED(CONFIG_BT_MESH_DEBUG_TRANS)
#include "mesh_main.h"
                      ...
                      const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    ALOGE("src 0x%04x dst 0x%04x app_idx 0x%04x sdu_len %u",
...
bool bt_mesh_tx_in_progress(void)
{
    ...
    for (i = 0; i < (int)MESH_ARRAY_SIZE(seg_tx); i++) {
...
static void seg_tx_reset(struct seg_tx *tx)
{
    ...
    if (bt_mesh.pending_update) {
        ALOGE("Proceding with pending IV Update");
...
static void seg_tx_send_unacked(struct seg_tx *tx)
{
    ...
    for (i = 0; i <= tx->seg_n; i++) {
        ...
        if (BT_MESH_ADV(seg)->busy) {
            ALOGE("Skipping segment that's still advertising");
            continue;
        }

        if (!(BT_MESH_ADV(seg)->seg.attempts--)) {
            ALOGE("Ran out of retransmit attempts");
            seg_tx_complete(tx, -ETIMEDOUT);
            return;
        }

        ALOGE("resending %u/%u", i, tx->seg_n);

        err = bt_mesh_net_resend(tx->sub, seg, tx->new_key,
                                 &seg_sent_cb, tx);
        if (err) {
            ALOGE("Sending segment failed");
                    ...
                    const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    ALOGE("src 0x%04x dst 0x%04x app_idx 0x%04x aszmic %u sdu_len %u",
           net_tx->src, net_tx->ctx->addr, net_tx->ctx->app_idx,
           net_tx->aszmic, sdu->len);

    if (sdu->len < 1) {
        ALOGE("Zero-length SDU not allowed");
        return -EINVAL;
    }

    if (sdu->len > BT_MESH_TX_SDU_MAX) {
        ALOGE("Not enough segment buffers for length %u", sdu->len);
        return -EMSGSIZE;
    }

    for (tx = NULL, i = 0; i < (int)MESH_ARRAY_SIZE(seg_tx); i++) {
        if (!seg_tx[i].nack_count) {
            tx = &seg_tx[i];
            break;
        }
    ...
    if (!tx) {
        ALOGE("No multi-segment message contexts available");
        return -EBUSY;
    }
    ...
    for (seg_o = 0; sdu->len; seg_o++) {
        ...
        ALOGE("Sending %u/%u", seg_o, tx->seg_n);
                       ...
                       const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    if (net_buf_simple_tailroom(msg) < 4) {
        ALOGE("Insufficient tailroom for Transport MIC");
        return -EINVAL;
    }
    ...
    ALOGE("net_idx 0x%04x app_idx 0x%04x dst 0x%04x", tx->sub->net_idx,
    ...
    if (tx->ctx->app_idx == BT_MESH_KEY_DEV) {
        ...
        if (!key) {
            ALOGE("%s: get device key fail", __func__);
            return -EINVAL;
        }
                    ...
                    struct net_buf_simple *buf)
{
    ...
    ALOGE("ASZMIC %u AKF %u AID 0x%02x", aszmic, AKF(&hdr), AID(&hdr));
    ALOGE("len %u: %s", buf->len, bt_hex(buf->data, buf->len));

    if (buf->len < 1 + APP_MIC_LEN(aszmic)) {
        ALOGE("Too short SDU + MIC");
        return -EINVAL;
    }

    if (IS_ENABLED(CONFIG_BT_MESH_FRIEND) && !rx->local_match) {
        ALOGE("Ignoring PDU for LPN 0x%04x of this Friend", rx->dst);
        return 0;
    }
    ...
    if (!AKF(&hdr)) {
        ...
        for (i = 0; i < array_size; i++) {
            ...
            if (!dev_key) {
                ALOGE("%s: get NULL dev_key", __func__);
                continue;
            }
            ...
            if (err) {
                ALOGE("Unable to decrypt with DevKey");
                continue;
            }
        ...
        ALOGE("%s: no dev_key found", __func__);
        return -ENODEV;
    }

#if CONFIG_BT_MESH_NODE && !CONFIG_BT_MESH_PROVISIONER
    if (!bt_mesh_is_provisioner_en()) {
        array_size = MESH_ARRAY_SIZE(bt_mesh.app_keys);
    }
#endif

#if !CONFIG_BT_MESH_NODE && CONFIG_BT_MESH_PROVISIONER
    if (bt_mesh_is_provisioner_en()) {
        array_size = MESH_ARRAY_SIZE(bt_mesh.p_app_keys);
    }
#endif
    ...
    for (i = 0; i < array_size; i++) {
...
#if CONFIG_BT_MESH_NODE && CONFIG_BT_MESH_PROVISIONER
        if (i < MESH_ARRAY_SIZE(bt_mesh.app_keys)) {
            key = &bt_mesh.app_keys[i];
        } else {
            key = bt_mesh.p_app_keys[i - MESH_ARRAY_SIZE(bt_mesh.app_keys)];
        }
#endif

        if (!key) {
            ALOGE("%s: get NULL app_key", __func__);
            continue;
        }
        ...
        err = bt_mesh_app_decrypt(keys->val, false, aszmic, buf,
                                  ...
                                  rx->seq, BT_MESH_NET_IVI_RX(rx));
        if (err) {
            ALOGE("Unable to decrypt with AppKey %u", i);
            continue;
        }
                     ...
                     struct net_buf_simple *buf, u64_t *seq_auth)
{
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_FRIEND) && rx->friend_match) {
        ALOGE("Ack for LPN 0x%04x of this Friend", rx->dst);
    ...
    ALOGE("OBO %u seq_zero 0x%04x ack 0x%08x", obo, seq_zero, ack);

    tx = seg_tx_lookup(seq_zero, obo, rx->ctx.addr);
    if (!tx) {
        ALOGE("No matching TX context for ack");
        return -EINVAL;
    }
    ...
    *seq_auth = tx->seq_auth;
    ...
    if (find_msb_set(ack) - 1 > tx->seg_n) {
        ALOGE("Too large segment number in ack");
        return -EINVAL;
    }
    ...
    while ((bit = find_lsb_set(ack))) {
        if (tx->seg[bit - 1]) {
            ALOGE("seg %u/%u acked", bit - 1, tx->seg_n);
                           ...
                           struct net_buf_simple *buf)
{
    ...
    if (buf->len < 3) {
        ALOGE("Too short heartbeat message");
        return -EINVAL;
    }

    if (rx->dst != hb_sub_dst) {
        ALOGE("Ignoring heartbeat to non-subscribed destination");
        return 0;
    }
    ...
    ALOGE("src 0x%04x TTL %u InitTTL %u (%u hop%s) feat 0x%04x",
                    ...
                    struct net_buf_simple *buf, u64_t *seq_auth)
{
    ...
    ALOGE("OpCode 0x%02x len %u", ctl_op, buf->len);
    ...
    if (bt_mesh_is_provisioned()) {
        ...
        if (rx->ctx.addr == bt_mesh.lpn.frnd) {
            ...
            if (!rx->friend_cred) {
                ALOGE("Message from friend with wrong credentials");
                return -EINVAL;
            }
    ...
    ALOGE("Unhandled TransOpCode 0x%02x", ctl_op);
                       ...
                       u64_t *seq_auth)
{
    ...
    ALOGE("AFK %u AID 0x%02x", AKF(buf->data), AID(buf->data));

    if (buf->len < 1) {
        ALOGE("Too small unsegmented PDU");
        return -EINVAL;
    }

    if (rx->local_match && is_replay(rx)) {
        ALOGE("Replay: src 0x%04x dst 0x%04x seq 0x%06x",
                     ...
                     const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    ALOGE("src 0x%04x dst 0x%04x ttl 0x%02x ctl 0x%02x", tx->src,
           tx->ctx->addr, tx->ctx->send_ttl, ctl_op);
    ALOGE("len %u: %s", data_len, bt_hex(data, data_len));

    buf = bt_mesh_adv_create(BT_MESH_ADV_DATA,
                             BT_MESH_TRANSMIT_COUNT(tx->xmit),
                             BT_MESH_TRANSMIT_INT(tx->xmit), BUF_TIMEOUT);
    if (!buf) {
        ALOGE("Out of transport buffers");
        return -ENOBUFS;
    }
                    ...
                    u8_t ttl, u64_t *seq_auth, u32_t block, u8_t obo)
{
    ...
    ALOGE("SeqZero 0x%04x Block 0x%08x OBO %u", seq_zero, block, obo);

    if (bt_mesh_lpn_established()) {
        ALOGE("Not sending ack when LPN is enabled");
        return 0;
    }
    ...
    if (!BT_MESH_ADDR_IS_UNICAST(src)) {
        ALOGE("Not sending ack for non-unicast address");
        return 0;
    }
...
static void seg_rx_reset(struct seg_rx *rx)
{
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_FRIEND) && rx->obo &&
        ...
        ALOGE("Clearing incomplete buffers from Friend queue");
...
static void seg_ack(struct k_work *work)
{
    ...
    if (k_uptime_get_32() - rx->last > (60 * MSEC_PER_SEC)) {
        ALOGE("Incomplete timer expired");
                                  ...
                                  const u64_t *seq_auth)
{
    ...
    for (i = 0; i < (int)MESH_ARRAY_SIZE(seg_rx); i++) {
        ...
        if (rx->in_use) {
            ALOGE("Duplicate SDU from src 0x%04x",
                    net_rx->ctx.addr);
                            ...
                            const u8_t *hdr, u8_t seg_n)
{
    if (rx->hdr != *hdr || rx->seg_n != seg_n) {
        ALOGE("Invalid segment for ongoing session");
        return false;
    }

    if (rx->src != net_rx->ctx.addr || rx->dst != net_rx->dst) {
        ALOGE("Invalid source or destination for segment");
        return false;
    }

    if (rx->ctl != net_rx->ctl) {
        ALOGE("Inconsistent CTL in segment");
        return false;
    }
                                   ...
                                   u8_t seg_n)
{
    ...
    for (i = 0; i < (int)MESH_ARRAY_SIZE(seg_rx); i++) {
        ...
        ALOGE("New RX context. Block Complete 0x%08x",
                     ...
                     enum bt_mesh_friend_pdu_type *pdu_type, u64_t *seq_auth)
{
    ...
    if (buf->len < 5) {
        ALOGE("Too short segmented message (len %u)", buf->len);
        return -EINVAL;
    }

    ALOGE("ASZMIC %u AKF %u AID 0x%02x", ASZMIC(hdr), AKF(hdr), AID(hdr));
    ...
    ALOGE("SeqZero 0x%04x SegO %u SegN %u", seq_zero, seg_o, seg_n);

    if (seg_o > seg_n) {
        ALOGE("SegO greater than SegN (%u > %u)", seg_o, seg_n);
        return -EINVAL;
    }
    ...
    *seq_auth = SEQ_AUTH(BT_MESH_NET_IVI_RX(net_rx),
    ...
    if (rx) {
        ...
        if (rx->in_use) {
            ALOGE("Existing RX context. Block 0x%08x", rx->block);
            goto found_rx;
        }

        if (rx->block == BLOCK_COMPLETE(rx->seg_n)) {
            ALOGE("Got segment for already complete SDU");
         ...
         */
        ALOGE("Got segment for canceled SDU");
        return -EINVAL;
    }

    /* Bail out early if we're not ready to receive such a large SDU */
    if (!sdu_len_is_ok(net_rx->ctl, seg_n)) {
        ALOGE("Too big incoming SDU length");
        send_ack(net_rx->sub, net_rx->dst, net_rx->ctx.addr,
    ...
    if (!rx) {
         ...
         * this one.
         */
        ALOGE("No free slots for new incoming segmented messages");
        return -ENOMEM;
    }
...
found_rx:
    if (BIT(seg_o) & rx->block) {
        ALOGE("Received already received fragment");
        return -EALREADY;
    }
    ...
    if (seg_o == seg_n) {
        ...
        ALOGE("Target len %u * %u + %u = %u", seg_n, seg_len(rx->ctl),
               buf->len, rx->buf.len);

        if (rx->buf.len > CONFIG_BT_MESH_RX_SDU_MAX) {
            ALOGE("Too large SDU len");
            send_ack(net_rx->sub, net_rx->dst, net_rx->ctx.addr,
        ...
        if (buf->len != seg_len(rx->ctl)) {
            ALOGE("Incorrect segment size for message type");
            return -EINVAL;
        }
    }
    ...
    ALOGE("Received %u/%u", seg_o, seg_n);
...
int bt_mesh_trans_recv(struct net_buf_simple *buf, struct bt_mesh_net_rx *rx)
{
    ...
    ALOGE("src 0x%04x dst 0x%04x seq 0x%08x friend_match %u",
           rx->ctx.addr, rx->dst, rx->seq, rx->friend_match);

    /* Remove network headers */
    net_buf_simple_pull(buf, BT_MESH_NET_HDR_LEN);

    ALOGE("Payload %s", bt_hex(buf->data, buf->len));
    ...
    if (bt_mesh_is_provisioned()) {
        if (IS_ENABLED(CONFIG_BT_MESH_LOW_POWER) &&
            ...
            ALOGE("Ignoring unexpected message in Low Power mode");
            return -EAGAIN;
        }
    }
#endif
