...
#define ALOGE_ENABLED IS_ENABLED(CONFIG_BT_MESH_DEBUG_MODEL)
...
static void hb_send(struct bt_mesh_model *model)
{
    ...
    ALOGE("InitTTL %u feat 0x%04x", cfg->hb_pub.ttl, feat);
                         ...
                         bool primary)
{
    ...
    if (net_buf_simple_tailroom(buf) <
            (unsigned int)(4 + (elem->model_count * 2) + (elem->vnd_model_count * 2))) {
        ALOGE("Too large device composition");
        return -E2BIG;
    }
...
static int comp_get_page_0(struct net_buf_simple *buf)
{
    ...
    for (i = 0; i < (int)comp->elem_count; i++) {
        int err;
                              ...
                              struct net_buf_simple *buf)
{
    ...
    ALOGE("net_idx 0x%04x app_idx 0x%04x src 0x%04x len %u: %s",
           ctx->net_idx, ctx->app_idx, ctx->addr, buf->len,
           bt_hex(buf->data, buf->len));

    page = net_buf_simple_pull_u8(buf);
    if (page != 0) {
        ALOGE("Composition page %u not available", page);
        page = 0;
    }
    ...
    if (comp_get_page_0(sdu) < 0) {
        ALOGE("Unable to get composition page 0");
        return;
    }

    if (bt_mesh_model_send(model, ctx, sdu, NULL, NULL)) {
        ALOGE("Unable to send Device Composition Status response");
    }
}
                                       ...
                                       struct net_buf_simple *buf, bool *vnd)
{
    if (buf->len < 4) {
        ...
        ALOGE("ID 0x%04x addr 0x%04x", id, elem->addr);
        ...
        *vnd = false;
        ...
        ALOGE("Company 0x%04x ID 0x%04x addr 0x%04x", company, id,
               elem->addr);
...
static bool app_key_is_valid(u16_t app_idx)
{
    ...
    for (i = 0; i < (int)MESH_ARRAY_SIZE(bt_mesh.app_keys); i++) {
...
static u8_t mod_bind(struct bt_mesh_model *model, u16_t key_idx)
{
    ...
    ALOGE("key_idx 0x%04x", key_idx);

    if (!app_key_is_valid(key_idx)) {
        return STATUS_INVALID_APPKEY;
    }

    for (i = 0; i < (int)MESH_ARRAY_SIZE(model->keys); i++) {
...
static u8_t mod_unbind(struct bt_mesh_model *model, u16_t key_idx)
{
    ...
    ALOGE("model %p key_idx 0x%04x", model, key_idx);
                        ...
                        bool update)
{
    ...
    ALOGE("net_idx 0x%04x app_idx %04x update %u val %s",
    ...
    ALOGE("app_idx 0x%04x AID 0x%02x", app_idx, keys->id);
                        ...
                        struct net_buf_simple *buf)
{
    ...
    ALOGE("AppIdx 0x%04x NetIdx 0x%04x", key_app_idx, key_net_idx);

    bt_mesh_model_msg_init(msg, OP_APP_KEY_STATUS);

    status = app_key_set(key_net_idx, key_app_idx, buf->data, false);
    ALOGE("status 0x%02x", status);
    net_buf_simple_add_u8(msg, status);

    key_idx_pack(msg, key_net_idx, key_app_idx);

    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send App Key Status response");
    }
}
                        ...
                        struct net_buf_simple *buf)
{
    ...
    if (get_idx > 0xfff) {
        ALOGE("Invalid NetKeyIndex 0x%04x", get_idx);
        return;
    }

    ALOGE("idx 0x%04x", get_idx);
...
send_status:
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send AppKey List");
    }
}
                       ...
                       struct net_buf_simple *buf)
{
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Config Beacon Status response");
    }
}
                       ...
                       struct net_buf_simple *buf)
{
    ...
    if (!cfg) {
        ALOGE("No Configuration Server context available");
        ...
        ALOGE("Invalid Config Beacon value 0x%02x", buf->data[0]);
        return;
    }
                            ...
                            struct net_buf_simple *buf)
{
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Default TTL Status response");
    }
}
                            ...
                            struct net_buf_simple *buf)
{
    ...
    if (!cfg) {
        ...
        ALOGE("Prohibited Default TTL value 0x%02x", buf->data[0]);
        return;
    }
                                   ...
                                   struct bt_mesh_msg_ctx *ctx)
{
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send GATT Proxy Status");
    }
}
                           ...
                           struct net_buf_simple *buf)
{
    ...
    if (buf->data[0] != 0x00 && buf->data[0] != 0x01) {
        ALOGE("Invalid GATT Proxy value 0x%02x", buf->data[0]);
        return;
    }
    ...
    ALOGE("GATT Proxy 0x%02x -> 0x%02x", cfg->gatt_proxy, buf->data[0]);
                             ...
                             struct net_buf_simple *buf)
{
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Config Network Transmit Status");
    }
}
                             ...
                             struct net_buf_simple *buf)
{
    ...
    ALOGE("Transmit 0x%02x (count %u interval %ums)", buf->data[0],
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Network Transmit Status");
    }
}
                      ...
                      struct net_buf_simple *buf)
{
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Config Relay Status response");
    }
}
                      ...
                      struct net_buf_simple *buf)
{
    ...
    if (!cfg) {
        ...
        ALOGE("Relay 0x%02x (%s) xmit 0x%02x (count %u interval %u)",
        ...
        ALOGE("Invalid Relay value 0x%02x", buf->data[0]);
        return;
    }
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Relay Status response");
    }
}
                                ...
                                u8_t status, u8_t *mod_id)
{
    ...
    if (bt_mesh_model_send(cfg_mod, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Model Publication Status");
    }
}
                        ...
                        struct net_buf_simple *buf)
{
    ...
    ALOGE("elem_addr 0x%04x", elem_addr);
                        ...
                        struct net_buf_simple *buf)
{
    ...
    if (pub_ttl > BT_MESH_TTL_MAX && pub_ttl != BT_MESH_TTL_DEFAULT) {
        ALOGE("Invalid TTL value 0x%02x", pub_ttl);
        return;
    }
    ...
    ALOGE("elem_addr 0x%04x pub_addr 0x%04x cred_flag %u",
           elem_addr, pub_addr, cred_flag);
    ALOGE("pub_app_idx 0x%03x, pub_ttl %u pub_period 0x%02x",
           pub_app_idx, pub_ttl, pub_period);
    ALOGE("retransmit 0x%02x (count %u interval %ums)", retransmit,
...
static u16_t va_find(u8_t *label_uuid, struct label **free_slot)
{
    ...
    for (i = 0; i < (int)MESH_ARRAY_SIZE(labels); i++) {
                                ...
                                bool vnd)
{
    ...
    ALOGE("status 0x%02x elem_addr 0x%04x sub_addr 0x%04x", status,
           elem_addr, sub_addr);
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Model Subscription Status");
    }
}
                        ...
                        struct net_buf_simple *buf)
{
    ...
    ALOGE("elem_addr 0x%04x, sub_addr 0x%04x", elem_addr, sub_addr);
    ...
    if (i == MESH_ARRAY_SIZE(mod->groups)) {
        status = STATUS_INSUFF_RESOURCES;
    } else {
        status = STATUS_SUCCESS;
                        ...
                        struct net_buf_simple *buf)
{
    ...
    ALOGE("addr 0x%04x id 0x%04x", addr, id);
...
send_list:
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Model Subscription List");
    }
}
                            ...
                            struct net_buf_simple *buf)
{
    ...
    ALOGE("addr 0x%04x company 0x%04x id 0x%04x", addr, company, id);
...
send_list:
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Vendor Model Subscription List");
    }
}
                                ...
                                u16_t idx, u8_t status)
{
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send NetKey Status");
    }
}
                        ...
                        struct net_buf_simple *buf)
{
    ...
    if (idx > 0xfff) {
        ALOGE("Invalid NetKeyIndex 0x%04x", idx);
        return;
    }

    ALOGE("idx 0x%04x", idx);

    sub = bt_mesh_subnet_get(idx);
    if (!sub) {
        int i;

        for (sub = NULL, i = 0; i < (int)MESH_ARRAY_SIZE(bt_mesh.sub); i++) {
                        ...
                        struct net_buf_simple *buf)
{
    ...
    if (del_idx > 0xfff) {
        ALOGE("Invalid NetKeyIndex 0x%04x", del_idx);
        return;
    }

    ALOGE("idx 0x%04x", del_idx);
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(bt_mesh.app_keys); i++) {
        struct bt_mesh_app_key *key = &bt_mesh.app_keys[i];
                        ...
                        struct net_buf_simple *buf)
{
    ...
    prev = BT_MESH_KEY_UNUSED;
    for (i = 0; i < MESH_ARRAY_SIZE(bt_mesh.sub); i++) {
        struct bt_mesh_subnet *sub = &bt_mesh.sub[i];
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send NetKey List");
    }
}
                              ...
                              struct net_buf_simple *buf)
{
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Node Identity Status");
    }
}
                              ...
                              struct net_buf_simple *buf)
{
    ...
    if (node_id != 0x00 && node_id != 0x01) {
        ALOGE("Invalid Node ID value 0x%02x", node_id);
        return;
    }
                         ...
                         struct net_buf_simple *buf)
{
    ...
    if (model == mod) {
        ALOGE("Client tried to bind AppKey to Configuration Model");
...
send_status:
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Model App Bind Status response");
    }
}
...
send_status:
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Model App Unbind Status response");
    }
}
...
send_list:
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Model Application List message");
    }
}
                       ...
                       struct net_buf_simple *buf)
{
     ...
     */
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Node Reset Status");
    }
                               ...
                               struct bt_mesh_msg_ctx *ctx)
{
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Friend Status");
    }
}
                       ...
                       struct net_buf_simple *buf)
{
    ...
    if (buf->data[0] != 0x00 && buf->data[0] != 0x01) {
        ALOGE("Invalid Friend value 0x%02x", buf->data[0]);
        return;
    }
    ...
    ALOGE("Friend 0x%02x -> 0x%02x", cfg->frnd, buf->data[0]);
                            ...
                            struct net_buf_simple *buf)
{
    ...
    ALOGE("net_idx 0x%04x app_idx 0x%04x src 0x%04x lpn_addr 0x%02x",
           ctx->net_idx, ctx->app_idx, ctx->addr, lpn_addr);

    if (!BT_MESH_ADDR_IS_UNICAST(lpn_addr)) {
        ALOGE("Invalid LPNAddress; ignoring msg");
        return;
    }
...
send_rsp:
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send LPN PollTimeout Status");
    }
}
                            ...
                            u16_t idx, u8_t phase, u8_t status)
{
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Key Refresh State Status");
    }
}
                    ...
                    struct net_buf_simple *buf)
{
    ...
    ALOGE("idx 0x%04x transition 0x%02x", idx, phase);
                               ...
                               struct hb_pub_param *orig_msg)
{
    ...
    ALOGE("src 0x%04x status 0x%02x", ctx->addr, status);
...
send:
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Heartbeat Publication Status");
    }
}
                              ...
                              struct net_buf_simple *buf)
{
    ...
    if (param->ttl > BT_MESH_TTL_MAX && param->ttl != BT_MESH_TTL_DEFAULT) {
        ALOGE("Invalid TTL value 0x%02x", param->ttl);
        return;
    }
    ...
    if (dst == BT_MESH_ADDR_UNASSIGNED) {
        ...
        ALOGE("period %u ms", hb_pwr2(param->period_log, 1) * 1000);
                               ...
                               struct bt_mesh_msg_ctx *ctx, u8_t status)
{
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        ALOGE("Unable to send Heartbeat Subscription Status");
    }
}
                              ...
                              struct net_buf_simple *buf)
{
    ...
    if (BT_MESH_ADDR_IS_VIRTUAL(sub_dst) || BT_MESH_ADDR_IS_RFU(sub_dst) ||
        ...
        ALOGE("Prohibited destination address");
        return;
    }

    if (sub_period > 0x11) {
        ALOGE("Prohibited subscription period 0x%02x", sub_period);
        return;
    }
...
static void hb_publish(struct k_work *work)
{
    ...
    ALOGE("hb_pub.count: %u", cfg->hb_pub.count);

    sub = bt_mesh_subnet_get(cfg->hb_pub.net_idx);
    if (!sub) {
        ALOGE("No matching subnet for idx 0x%02x",
               cfg->hb_pub.net_idx);
        cfg->hb_pub.dst = BT_MESH_ADDR_UNASSIGNED;
        return;
    }
...
int bt_mesh_cfg_srv_init(struct bt_mesh_model *model, bool primary)
{
    ...
    if (!cfg) {
        ALOGE("No Configuration Server context provided");
        return -EINVAL;
    }

    ALOGE("%s, net_transmit = %d, relay = %d, relay_retransmit = %d, beacon = %d, gatt_proxy = %d, frnd = %d, default_ttl = %d",
    ...
    if (!conf_is_valid(cfg)) {
        ALOGE("Invalid values in configuration");
        return -EINVAL;
    }
...
void bt_mesh_heartbeat(u16_t src, u16_t dst, u8_t hops, u16_t feat)
{
    ...
    if (!cfg) {
        ALOGE("No configuaration server context available");
        return;
    }

    if (src != cfg->hb_sub.src || dst != cfg->hb_sub.dst) {
        ALOGE("No subscription for received heartbeat");
        return;
    }

    if (k_uptime_get() > cfg->hb_sub.expiry) {
        ALOGE("Heartbeat subscription period expired");
        return;
    }
    ...
    ALOGE("src 0x%04x dst 0x%04x hops %u min %u max %u count %u", src,
...
{
    if (conf) {
        ALOGE("conf %p conf->frnd 0x%02x", conf, conf->frnd);
        return conf->frnd;
    }
...
u8_t *bt_mesh_label_uuid_get(u16_t addr)
{
    ...
    for (i = 0; i < (int)MESH_ARRAY_SIZE(labels); i++) {
        if (labels[i].addr == addr) {
            ALOGE("Found Label UUID for 0x%04x: %s", addr,
                   bt_hex(labels[i].uuid, 16));
            return labels[i].uuid;
        }
    }

    ALOGE("No matching Label UUID for 0x%04x", addr);
