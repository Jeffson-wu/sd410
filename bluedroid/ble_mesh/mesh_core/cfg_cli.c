...
static void timeout_handler(struct k_work *work)
{
    ...
    ALOGE("Receive configuration status message timeout");
                              ...
                              void *status, size_t len)
{
    ...
    if (!data) {
        ALOGE("%s: config client internal_data is NULL", __func__);
        return;
    }
    ...
    if (!node) {
        ALOGE("Unexpected config status message 0x%x", ctx->recv_op);
    } else {
        switch (node->opcode) {
        case OP_BEACON_GET:
                             ...
                             struct net_buf_simple *buf)
{
    ...
    if (!status.comp_data) {
        ALOGE("%s: allocate memory for comp_data fail", __func__);
        return;
    }
...
int bt_mesh_cfg_comp_data_get(struct bt_mesh_msg_ctx *ctx, u8_t page)
{
    ...
    err = bt_mesh_client_send_msg(cli->model, OP_DEV_COMP_DATA_GET, ctx,
                                  ...
                                  true, NULL, NULL);
    if (err) {
        ALOGE("%s: send failed (err %d)", __func__, err);
    }
                      ...
                      const u8_t label[16], u16_t mod_id, u16_t cid)
{
    ...
    ALOGE("net_idx 0x%04x addr 0x%04x elem_addr 0x%04x label %s",
           ctx->net_idx, ctx->addr, elem_addr, label);
    ALOGE("mod_id 0x%04x cid 0x%04x", mod_id, cid);
...
int bt_mesh_cfg_cli_init(struct bt_mesh_model *model, bool primary)
{
    ...
    if (!primary) {
        ALOGE("Configuration Client only allowed in primary element");
        return -EINVAL;
    }

    if (!model) {
        ALOGE("Configuration Client model is NULL");
        return -EINVAL;
    }

    client = (bt_mesh_config_client_t *)model->user_data;
    if (!client) {
        ALOGE("No Configuration Client context provided");
        return -EINVAL;
    }

    /* TODO: call free() when deinit function is invoked*/
    internal = malloc(sizeof(config_internal_data_t));
    if (!internal) {
        ALOGE("Allocate memory for Configuration Client internal data fail");
        return -ENOMEM;
    }
    ...
    client->model         = model;
    client->op_pair_size  = MESH_ARRAY_SIZE(cfg_op_pair);
