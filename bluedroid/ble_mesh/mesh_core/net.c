...
#define ALOGE_ENABLED IS_ENABLED(CONFIG_BT_MESH_DEBUG_NET)
...
static bool check_dup(struct net_buf_simple *data)
{
    ...
    for (i = 0; i < (int)MESH_ARRAY_SIZE(dup_cache); i++) {
        if (dup_cache[i] == val) {
            return true;
        }
    }
    ...
    dup_cache_next %= MESH_ARRAY_SIZE(dup_cache);
                            ...
                            struct net_buf_simple *pdu)
{
    ...
    for (i = 0; i < (int)MESH_ARRAY_SIZE(msg_cache); i++) {
        if (msg_cache[i] == hash) {
            return true;
        }
    }
    ...
    msg_cache_next %= MESH_ARRAY_SIZE(msg_cache);
                            ...
                            const u8_t key[16])
{
    ...
    if (err) {
        ALOGE("Unable to generate NID, EncKey & PrivacyKey");
        return err;
    }
    ...
    ALOGE("NID 0x%02x EncKey %s", keys->nid, bt_hex(keys->enc, 16));
    ALOGE("PrivacyKey %s", bt_hex(keys->privacy, 16));

    err = bt_mesh_k3(key, keys->net_id);
    if (err) {
        ALOGE("Unable to generate Net ID");
        return err;
    }

    ALOGE("NetID %s", bt_hex(keys->net_id, 8));

#if defined(CONFIG_BT_MESH_GATT_PROXY)
    err = bt_mesh_identity_key(key, keys->identity);
    if (err) {
        ALOGE("Unable to generate IdentityKey");
        return err;
    }

    ALOGE("IdentityKey %s", bt_hex(keys->identity, 16));
#endif /* GATT_PROXY */

    err = bt_mesh_beacon_key(key, keys->beacon);
    if (err) {
        ALOGE("Unable to generate beacon key");
        return err;
    }

    ALOGE("BeaconKey %s", bt_hex(keys->beacon, 16));
...
int friend_cred_set(struct friend_cred *cred, u8_t idx, const u8_t net_key[16])
{
    ...
    ALOGE("LPNAddress 0x%04x FriendAddress 0x%04x", lpn_addr, frnd_addr);
    ALOGE("LPNCounter 0x%04x FriendCounter 0x%04x", cred->lpn_counter,
           cred->frnd_counter);
    ...
    ALOGE("Friend NID 0x%02x EncKey %s", cred->cred[idx].nid,
           bt_hex(cred->cred[idx].enc, 16));
    ALOGE("Friend PrivacyKey %s", bt_hex(cred->cred[idx].privacy, 16));
...
void friend_cred_refresh(u16_t net_idx)
{
    ...
    for (i = 0; i < (int)MESH_ARRAY_SIZE(friend_cred); i++) {
...
int friend_cred_update(struct bt_mesh_subnet *sub)
{
    ...
    ALOGE("net_idx 0x%04x", sub->net_idx);
                                       ...
                                       u16_t lpn_counter, u16_t frnd_counter)
{
    ...
    ALOGE("net_idx 0x%04x addr 0x%04x", sub->net_idx, addr);

    for (cred = NULL, i = 0; i < (int)MESH_ARRAY_SIZE(friend_cred); i++) {
    ...
    if (!cred) {
        ALOGE("No free friend credential slots");
        return NULL;
    }
...
int bt_mesh_net_beacon_update(struct bt_mesh_subnet *sub)
{
    ...
    if (sub->kr_flag) {
        ALOGE("NetIndex %u Using new key", sub->net_idx);
        keys = &sub->keys[1];
    } else {
        ALOGE("NetIndex %u Using current key", sub->net_idx);
        keys = &sub->keys[0];
    }

    ALOGE("flags 0x%02x, IVI 0x%08x", flags, bt_mesh.iv_index);
                       ...
                       u32_t iv_index)
{
    ...
    ALOGE("idx %u flags 0x%02x iv_index %u", idx, flags, iv_index);

    ALOGE("NetKey %s", bt_hex(key, 16));
...
bool bt_mesh_kr_update(struct bt_mesh_subnet *sub, u8_t new_kr, bool new_key)
{
    if (new_kr != sub->kr_flag && sub->kr_phase == BT_MESH_KR_NORMAL) {
        ALOGE("KR change in normal operation. Are we blacklisted?");
        return false;
    }
    ...
    if (sub->kr_flag) {
        ...
        case BT_MESH_KR_PHASE_2:
            ALOGE("KR Phase 0x%02x -> Normal", sub->kr_phase);
...
void bt_mesh_rpl_reset(void)
{
     ...
     */
    for (i = 0; i < (int)MESH_ARRAY_SIZE(bt_mesh.rpl); i++) {
...
bool bt_mesh_net_iv_update(u32_t iv_index, bool iv_update)
{
    ...
    if (bt_mesh.iv_update) {
        ...
        if (iv_index != bt_mesh.iv_index) {
            ALOGE("IV Index mismatch: 0x%08x != 0x%08x",
        ...
        if (iv_update) {
            /* Nothing to do */
            ALOGE("Already in IV Update in Progress state");
            return false;
        }
    } else {
        ...
        if (iv_index == bt_mesh.iv_index) {
            ALOGE("Same IV Index in normal mode");
            return false;
        }

        if (iv_index < bt_mesh.iv_index ||
                iv_index > bt_mesh.iv_index + 42) {
            ALOGE("IV Index out of sync: 0x%08x != 0x%08x",
                   iv_index, bt_mesh.iv_index);
            return false;
        }

        if (iv_index > bt_mesh.iv_index + 1) {
            ALOGE("Performing IV Index Recovery");
        ...
        if (iv_index == bt_mesh.iv_index + 1 && !iv_update) {
            ALOGE("Ignoring new index in normal mode");
            return false;
        }

        if (!iv_update) {
            /* Nothing to do */
            ALOGE("Already in Normal state");
            return false;
        }

        if (iv_index != bt_mesh.iv_index + 1) {
            ALOGE("Wrong new IV Index: 0x%08x != 0x%08x + 1",
                    iv_index, bt_mesh.iv_index);
            return false;
        }
    }
    ...
    if (!IS_ENABLED(CONFIG_BT_MESH_IV_UPDATE_TEST) || !bt_mesh.ivu_test) {
        ...
        if (delta < K_HOURS(96)) {
            ALOGE("IV Update before minimum duration");
            return false;
        }
    }
    ...
    if (!iv_update && bt_mesh_tx_in_progress()) {
        ALOGE("IV Update deferred because of pending transfer");
...
do_update:
    ...
    if (bt_mesh.iv_update) {
        bt_mesh.iv_index = iv_index;
        ALOGE("IV Update state entered. New index 0x%08x",
               bt_mesh.iv_index);
                       ...
                       void *cb_data)
{
    ...
    ALOGE("net_idx 0x%04x new_key %u len %u", sub->net_idx, new_key,
           buf->len);
    ...
    if (err) {
        ALOGE("deobfuscate failed (err %d)", err);
        return err;
    }

    err = bt_mesh_net_decrypt(enc, &buf->b, BT_MESH_NET_IVI_TX, false);
    if (err) {
        ALOGE("decrypt failed (err %d)", err);
        return err;
    }
    ...
    if (err) {
        ALOGE("encrypt failed (err %d)", err);
        return err;
    }

    err = bt_mesh_net_obfuscate(buf->data, BT_MESH_NET_IVI_TX, priv);
    if (err) {
        ALOGE("obfuscate failed (err %d)", err);
        return err;
    }
                       ...
                       bool proxy)
{
    ...
    if (ctl && net_buf_simple_tailroom(buf) < 8) {
        ALOGE("Insufficient MIC space for CTL PDU");
        return -EINVAL;
    } else if (net_buf_simple_tailroom(buf) < 4) {
        ALOGE("Insufficient MIC space for PDU");
        return -EINVAL;
    }

    ALOGE("src 0x%04x dst 0x%04x ctl %u seq 0x%06x",
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_LOW_POWER) && tx->friend_cred) {
        if (friend_cred_get(tx->sub, BT_MESH_ADDR_UNASSIGNED,
                            &nid, &enc, &priv)) {
            ALOGE("Falling back to master credentials");
                     ...
                     const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    ALOGE("src 0x%04x dst 0x%04x len %u headroom %u tailroom %u",
           tx->src, tx->ctx->addr, buf->len, net_buf_headroom(buf),
           net_buf_tailroom(buf));
    ALOGE("Payload len %u: %s", buf->len, bt_hex(buf->data, buf->len));
    ALOGE("Seq 0x%06x", bt_mesh.seq);
                       ...
                       u32_t iv_index, const u8_t auth[8])
{
    ...
    if (memcmp(auth, net_auth, 8)) {
        ALOGE("Authentication Value %s != %s",
...
{
    ALOGE("NID 0x%02x net_idx 0x%04x", NID(data), sub->net_idx);
    ALOGE("IVI %u net->iv_index 0x%08x", IVI(data), bt_mesh.iv_index);
    ...
    if (rx->net_if == BT_MESH_NET_IF_ADV && msg_cache_match(rx, buf)) {
        ALOGE("Duplicate found in Network Message Cache");
        return -EALREADY;
    }

    rx->ctx.addr = SRC(buf->data);
    if (!BT_MESH_ADDR_IS_UNICAST(rx->ctx.addr)) {
        ALOGE("Ignoring non-unicast src addr 0x%04x", rx->ctx.addr);
        return -EINVAL;
    }

    ALOGE("src 0x%04x", rx->ctx.addr);
                          ...
                          struct net_buf_simple *buf)
{
    ...
    for (i = 0; i < MESH_ARRAY_SIZE(friend_cred); i++) {
        struct friend_cred *cred = &friend_cred[i];
                                 ...
                                 struct net_buf_simple *buf)
{
    ...
    if (!bt_mesh_is_provisioner_en()) {
        array_size = MESH_ARRAY_SIZE(bt_mesh.sub);
    }
#endif

#if !CONFIG_BT_MESH_NODE && CONFIG_BT_MESH_PROVISIONER
    if (bt_mesh_is_provisioner_en()) {
        array_size = MESH_ARRAY_SIZE(bt_mesh.p_sub);
    }
#endif
    ...
    if (!array_size) {
        ALOGE("%s: Get sub array_size fail", __func__);
        return false;
    }
...
#if CONFIG_BT_MESH_NODE && CONFIG_BT_MESH_PROVISIONER
        if (i < MESH_ARRAY_SIZE(bt_mesh.sub)) {
            sub = &bt_mesh.sub[i];
        } else {
            sub = bt_mesh.p_sub[i - MESH_ARRAY_SIZE(bt_mesh.sub)];
        }
#endif

        if (!sub) {
            ALOGE("%s: get NULL sub", __func__);
            continue;
        }
                              ...
                              struct bt_mesh_net_rx *rx)
{
    ...
    ALOGE("TTL %u CTL %u dst 0x%04x", rx->ctx.recv_ttl, rx->ctl, rx->dst);
    ...
    ALOGE("Relaying packet. TTL is now %u", TTL(buf->data));
    ...
    if (bt_mesh_net_encrypt(enc, &buf->b, BT_MESH_NET_IVI_RX(rx), false)) {
        ALOGE("Re-encrypting failed");
        goto done;
    }

    if (bt_mesh_net_obfuscate(buf->data, BT_MESH_NET_IVI_RX(rx), priv)) {
        ALOGE("Re-obfuscating failed");
        goto done;
    }
                       ...
                       struct bt_mesh_net_rx *rx, struct net_buf_simple *buf)
{
    if (data->len < BT_MESH_NET_MIN_PDU_LEN) {
        ALOGE("Dropping too short mesh packet (len %u)", data->len);
        ALOGE("%s", bt_hex(data->data, data->len));
        return -EINVAL;
    }
    ...
    ALOGE("%u bytes: %s", data->len, bt_hex(data->data, data->len));

    rx->net_if = net_if;

    if (!net_find_and_decrypt(data->data, data->len, rx, buf)) {
        ALOGE("Unable to find matching net for packet");
        return -ENOENT;
    }
    ...
    ALOGE("Decryption successful. Payload len %u", buf->len);

    if (net_if != BT_MESH_NET_IF_PROXY_CFG &&
            rx->dst == BT_MESH_ADDR_UNASSIGNED) {
        ALOGE("Destination address is unassigned; dropping packet");
        return -EBADMSG;
    }

    if (BT_MESH_ADDR_IS_RFU(rx->dst)) {
        ALOGE("Destination address is RFU; dropping packet");
        return -EBADMSG;
    }

    if (net_if != BT_MESH_NET_IF_LOCAL && bt_mesh_elem_find(rx->ctx.addr)) {
        ALOGE("Dropping locally originated packet");
        return -EBADMSG;
    }

    ALOGE("src 0x%04x dst 0x%04x ttl %u", rx->ctx.addr, rx->dst,
           rx->ctx.recv_ttl);
    ALOGE("PDU: %s", bt_hex(buf->data, buf->len));
                      ...
                      enum bt_mesh_net_if net_if)
{
    ...
    ALOGE("rssi %d net_if %u", rssi, net_if);

#if CONFIG_BT_MESH_NODE
    if (!bt_mesh_is_provisioner_en()) {
        if (!bt_mesh_is_provisioned()) {
            ALOGE("%s: not provisioned", __func__);
            return;
        }
    }
#endif
...
#if !CONFIG_BT_MESH_NODE && CONFIG_BT_MESH_PROVISIONER
    if (!bt_mesh_is_provisioner_en()) {
        ALOGE("%s: provisioner disabled", __func__);
        return;
    }
    if (!provisioner_get_prov_node_count()) {
        ALOGE("%s: no device provisioned", __func__);
        return;
    }
#endif
