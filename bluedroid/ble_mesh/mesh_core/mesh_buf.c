...
//{
//    ALOGE("%s, the pool id = %p", __func__, pool);
        ...
        u16_t uninit_count)
{
    ...
    ALOGE("%s, pool_id =  %p, uninit_count = %d, pool->buf_count = %d, buf = %p, buf->size = %d, pool = %p",
...
#endif
{
    ...
    while (buf) {
        ...
        pool->avail_count++;
        ALOGE("%s, pool->avail_count = %d, pool->uninit_count = %d", __func__,
...
#endif
{
    ...
    ALOGE("%s, pool = %p , pool->uninit_count= %d, buf_count = %d", __func__,
    ...
    if (!buf) {
        ALOGE("%s():Failed to get free buffer", __func__);
    }
    return buf;
